...
int P2Pnet::Init()
{
  ...
  for(int i=0; i<max_batch; i++){
    auto ret= bm_image_create(m_bmContext->handle(), m_net_h, m_net_w, 
      FORMAT_RGB_PLANAR, DATA_TYPE_EXT_1N_BYTE, &m_resized_imgs[i], strides);
...
void P2Pnet::enableProfile(TimeStamp *ts)
{
  m_ts = ts;
}

int P2Pnet::batch_size() {
  return max_batch;
};
...
float P2Pnet::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool *pIsAligWidth)
{
  float ratio;
  ...
  std::vector<PPointVec>& points)
{
  ...
  for(int batch_idx = 0; batch_idx < images.size(); ++ batch_idx)
  {
    ...
    std::cout<<"frame size: "<<frame_width<<" "<<frame_height<<std::endl;
    ...
    float *score_data = (float*)outputTensors[0]->get_cpu_data() + 
      batch_idx*score_c*score_h*score_w;
    ...
    float *point_data = (float*)outputTensors[1]->get_cpu_data() + 
      batch_idx*point_c*point_h*point_w;

    PPointVec point_vec;
    point_vec.clear();
    std::cout << "score: " << score_c << ", " << score_h << ", " << score_w << std::endl;
    std::cout << "point: " << point_c << ", " << point_h << ", " << point_w << std::endl;
    CV_Assert(score_h == point_h && score_w == point_w);

    for (int hi = 0; hi < score_h; hi++)
    {
      float *pscore = score_data + hi * score_w;
      float *ppoint = point_data + hi * score_w;
      // std::cout<<pscore[0]<<" "<<pscore[1]<<std::endl;
      ...
      // std::cout<<pscore[1]<<" "<<x<<" "<<y<<std::endl;
