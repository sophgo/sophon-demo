...
function gen_int8bmodel()
{
	python3 -m ufw.cali.cali_model  \
		...
		--cali_iterations=100   \
		--cali_image_preprocess="resize_h=${img_height},resize_w=${img_width};scale=0.003921569,bgr2rgb=True"   \
...
if [ $? -eq 0 ]; then
    cp ${int8model_dir}/${dst_model_prefix}_batch${batch_size}/compilation.bmodel ${int8model_dir}/../${dst_model_prefix}_${platform}_int8_${batch_size}b.bmodel
    echo "Congratulation! int8bmodel is done!"
else
