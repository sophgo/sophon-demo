...
import cv2
import argparse
from p2pnet_utils.onnx_inference import OnnxInference
    ...
    if os.path.isdir(opt.input) or is_img(opt.input):
        ...
        for ino in range(img_num):
            ...
            for i, (e_img, p) in enumerate(zip(images, points)):
                ...
                save_basename = 'res_onnx_{}'.format(os.path.basename(input_batch[i]))
...
def parse_opt():
    ...
    model_path = os.path.join(os.path.dirname(__file__),"../models/onnx/p2pnet_1b.onnx")
