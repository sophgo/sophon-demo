...
​SAM2是Meta基于SAM提出的一种实时图像和视频分割模型。SAM2适用于图像和视频。而SAM的先前版本是专门为图像使用而构建的。本例程对[​SAM2官方开源仓库](https://github.com/facebookresearch/segment-anything-2)的模型和算法进行移植，使之能在SOPHON BM1684X/CV84X6/BM1688上进行推理测试。
...
### 2.2 SDK特性
* 支持BM1684X(x86 PCIe、SoC、riscv PCIe)、CV84X6(SoC)、BM1688(SoC)
* 图像编码器（Image Encoder）部分支持FP16、FP32的模型编译和推理，支持1core和2core(BM1688)；CV84X6支持FP16的模型编译和推理
* 图像解码器（Image Decoder）部分支持FP16、FP32的模型编译和推理，支持1core和2core(BM1688)；CV84X6支持FP16的模型编译和推理
...
CV84X6平台请使用：
```bash
./scripts/gen_bmodel_image.sh --chip cv84x6 --mode f16
```

执行上述命令会在`models/CV84X6/image_encoder`下生成`sam2_encoder_f16_1b.bmodel`，在`models/CV84X6/image_decoder`下生成`sam2_decoder_f16_1b.bmodel`文件，即CV84X6平台转换好的图像编码和解码的FP16 BModel。
**注意：CV84X6当前固件codegen不支持FP32（含matmul的模型编译时tpuc-opt断言失败），且INT8量化校准时TPU-MLIR解释器需同时驻留全部中间激活（峰值约28GB主机内存），常规环境会OOM被杀，CV84X6请使用FP16 BModel。**
...
CV84X6平台请使用：
```bash
./scripts/gen_bmodel_video.sh --chip cv84x6 --mode f16
```

执行上述命令会在`models/CV84X6/video`下生成sam2_image_encoder_no_pos.bmodel、sam2_image_decoder.bmodel、sam2_memory_attention_nomatmul.bmodel和sam2_memory_encoder.bmodel。视频分割推理还需要pos.npz和maskmem_pos_enc.npz两个常量文件（下载脚本会一并下载到`models/CV84X6/video`下）。
...
|   SE13-64     | CV84X6/image_encoder/sam2_encoder_f16_1b.bmodel          |         153.43     |
|   SE13-64     | CV84X6/image_decoder/sam2_decoder_f16_1b.bmodel          |           5.63     |
|   SE13-64     | CV84X6/video/sam2_image_encoder_no_pos.bmodel            |         152.99     |
|   SE13-64     | CV84X6/video/sam2_image_decoder.bmodel                   |          23.48     |
|   SE13-64     | CV84X6/video/sam2_memory_attention_nomatmul.bmodel       |        2168.02     |
|   SE13-64     | CV84X6/video/sam2_memory_encoder.bmodel                  |          13.52     |
...
> 3. SE13系列为CV84X6平台，目前仅提供fp16 bmodel（fp32/int8编译受限，见3.2节说明）。
