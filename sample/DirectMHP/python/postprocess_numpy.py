...
class PostProcess:
    def __init__(self, conf_thresh=0.001, nms_thresh=0.65, agnostic=False, multi_label=False, max_det=300):
...
class pseudo_torch_nms:
    ...
    def non_max_suppression(self,
        ...
        Returns:
             list of detections, on (n,9) tensor per image [xyxy, conf, cls, p, y, r]
        """
        ...
        output = [np.zeros((0, 6 + num_angles))] * bs
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            x[:, 5:-num_angles] *= x[:, 4:5]  # conf = obj_conf * cls_co
            ...
            if multi_label:
                i, j = (x[:, 5:-num_angles] > conf_thres).nonzero()
                x = np.concatenate([box[i], x[i, j + 5, None], j[:, None].astype(np.float32), x[i, -num_angles:]], 1)
            else:  # best class only
                conf = x[:, 5:-num_angles].max(1, keepdims=True)
                j_argmax = x[:, 5:-num_angles].argmax(1)
                j = j_argmax if j_argmax.shape == x[:, 5:-num_angles].shape else \
                    np.expand_dims(j_argmax, 1)  # for argmax(axis, keepdims=True)
                x = np.concatenate([box, conf, j.astype(np.float32), x[:, -num_angles:]], 1)[conf.reshape(-1) > conf_thres]
