...
int main(int argc, char *argv[]){
  cout.setf(ios::fixed);
  // get params
  const char *keys="{bmodel | ../../models/BM1684X/directmhp_fp32_1b.bmodel | bmodel file path}"
    "{dev_id | 0 | TPU device id}"
    "{conf_thresh | 0.001 | confidence threshold for filter boxes}"
    "{nms_thresh | 0.65 | iou threshold for nms}"
  ...
  // initialize net
  DirectMHP directmhp(bm_ctx);
  CV_Assert(0 == directmhp.Init(
        parser.get<float>("conf_thresh"),
        parser.get<float>("nms_thresh")
        ));

  // profiling
  TimeStamp directmhp_ts;
  TimeStamp *ts = &directmhp_ts;
  directmhp.enableProfile(&directmhp_ts);

  // get batch_size
  int batch_size = directmhp.batch_size();  
  ...
  if (info.st_mode & S_IFDIR){
    ...
    for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++){
      ...
      if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
        // predict
        CV_Assert(0 == directmhp.Detect(batch_imgs, boxes));
        ...
        for(int i = 0; i < batch_imgs.size(); i++){
          ...
          for (auto bbox : boxes[i]) {
            ...
            directmhp.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height, bbox.pitch, bbox.roll, bbox.yaw, batch_imgs[i],false);

            // save result
            json bbox_json;
            bbox_json["image_name"] = batch_names[i];
            bbox_json["category_id"] = 1;
            bbox_json["bbox"] = {bbox.x, bbox.y, bbox.width, bbox.height};
            bbox_json["score"] = bbox.score;
            bbox_json["pitch"] = bbox.pitch,
            bbox_json["yaw"] = bbox.yaw;
            bbox_json["roll"] = bbox.roll;
...
// test video
  ...
  directmhp_ts.calbr_basetime(base_time);
  directmhp_ts.build_timeline("directmhp test");
  directmhp_ts.show_summary("directhmp test");
  directmhp_ts.clear();
