...
int main(int argc, char* argv[]) {
    ...
    const char* keys =
        ...
        "{vision | ../../models/BM1684X/vision_embedding_6batch.bmodel | path of vision_embedding model}"
    ...
    // init
    auto vila = VILA(input, devids, llm_bmodel_file, vision_bmodel_file, tokenizer_path);
    ...
    while (true) {
        ...
        for (int i = 0; i < vila.get_num_frames(); i++)
            images_prompt += "<image>\n";
        ...
        int token = vila.forward_first(tokens);
