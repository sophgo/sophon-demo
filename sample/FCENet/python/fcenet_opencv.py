...
class FCENet(object):
    def __init__(self, args):
        ...
        self.is_static = True
        if self.net_h==2304: # static shape:[736,992], dynamic shape: [2304,2304]
        ...
        logging.info('input_dtype: {}, input_bm_image_data_format: {}, output_dtype: {}'.format(str(self.input_dtype), str(self.img_dtype), str(self.output_dtype)))
    ...
    def preprocess(self, img):
        img_tmp = np.expand_dims(np.transpose(img, (2, 0, 1)), axis=0)

        src_h, src_w, _ = img.shape
        if self.is_static:
            if src_h != self.net_h or src_w != self.net_w:
...
def main(args):
    ...
    # save result
    result_file = os.path.split(args.bmodel)[-1] + "_" + os.path.split(args.input_path)[-1] + "_opencv" + "_python_result.json"
