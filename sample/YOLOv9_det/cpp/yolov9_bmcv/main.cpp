...
int main(int argc, char *argv[]){
    cout.setf(ios::fixed);
    // get params
    const char *keys="{bmodel | ../../models/BM1684X/yolov9s_fp32_1b.bmodel | bmodel file path}"
                ...
                for(int i = 0; i < batch_size; i++){
                    ...
                    if(boxes[i].size() != 0){
                        for (auto bbox : boxes[i]) {
                            ...
                            // draw image
                            if(bbox.score > 0.25)
                                yolov9.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x1, bbox.y1, bboxwidth, bboxheight, batch_imgs[i]);
                            // save result
                            json bbox_json;
