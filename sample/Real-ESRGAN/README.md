...
## 1. 简介
本例程对[Real-ESRGAN](https://github.com/xinntao/Real-ESRGAN)的`realesr-general-x4v3`轻量级超分模型进行移植，使之能在SOPHON BM1684X/CV84X6/BM1688/CV186X 上进行推理测试。
...
### 2.2 SDK特性
* 支持BM1688(SoC)、BM1684X(x86 PCIe、SoC、riscv PCIe)和CV84X6(SoC)
* 支持FP32、FP16(BM1684X/BM1688/CV84X6)、INT8模型编译和推理
* 支持C++、Python推理
* 支持图片测试
...
├── CV84X6
│   ├── real_esrgan_fp16_1b.bmodel   # 使用TPU-MLIR编译，用于CV84X6的FP16 BModel，batch_size=1
│   ├── real_esrgan_int8_1b.bmodel   # 使用TPU-MLIR编译，用于CV84X6的INT8 BModel，batch_size=1
│   └── real_esrgan_int8_4b.bmodel   # 使用TPU-MLIR编译，用于CV84X6的INT8 BModel，batch_size=4
├── BM1688
...
​本例程在`scripts`目录下提供了TPU-MLIR编译FP32 BModel的脚本，请注意修改`gen_fp32bmodel_mlir.sh`中的onnx模型路径、生成模型目录和输入大小shapes等参数，并在执行时指定BModel运行的目标平台（**支持BM1684X/BM1688/CV186X**；CV84X6当前固件codegen不支持FP32，请使用FP16/INT8模型），如：
...
> 2. SE13-64对应CV84X6，当前固件codegen不支持FP32，测试使用FP16/INT8模型；
> 3. 早期版本bmcv.soc在000000000081.jpg上PSNR偏低（23.53），经定位为CV84X6硬件JPEG解码器固件失真（C++例程`cv::imread`走VPU硬解码路径）；libsophon 0.4.13修复该固件问题后复测，081.jpg PSNR恢复至36.76，bmcv.soc平均PSNR（36.76）与opencv.py（36.80）基本一致；
...
|   SE13-64   | CV84X6/real_esrgan_fp16_1b.bmodel |         189.85  |
|   SE13-64   | CV84X6/real_esrgan_int8_1b.bmodel |          85.48  |
|   SE13-64   | CV84X6/real_esrgan_int8_4b.bmodel |         340.50  |
...
> 5. SE13-64对应CV84X6（SoC）：FP16模型输出为FP32，后处理走CPU路径（`post_process_opencv`），耗时较长（约117ms）；INT8模型输出为UINT8，后处理走bmcv零拷贝路径（`post_process_bmcv`），仅需约6.4ms；int8_4b的推理时间为折算后单张耗时；bmcv.soc的decode time因图片仅4张（统计无效）未列出。
