...
char* rleToString(vector<int> R) {
  ...
  unsigned long p=0; long x; int more;
  unsigned long m=R.size();
  char *s=(char*)malloc(sizeof(char)*m*6);
  for(int i=0; i<m; i++ ) {
    x=(long) R[i]; if(i>2) x-=(long) R[i-2]; more=1;
    while( more ) {
...
json runLengthEncode(const cv::Mat ucharMat,int x1, int y1,int bboxw, int bboxh,int source_w,int source_h) {
    ...
    for (int row = 0; row < ucharMat.cols; ++row) {
        ...
        for (int col = 0; col < ucharMat.rows; ++col) {
            ...
            if(pixelValue==currentPixel) sum++;
            else {
        ...
        if(row != ucharMat.cols-1){
            if(currentPixel==false) sum+=source_h-bboxh;
            else {
                result.push_back(sum);
                currentPixel=false;
                sum=source_h-bboxh;
            ...
            if(currentPixel==false) sum+=source_h-bboxh-y1;
            else {
                result.push_back(sum);
                currentPixel=false;
            ...
            if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
                ...
                for(int i = 0; i < batch_imgs.size(); i++){
                    ...
                    for (auto bbox : boxes[i]) {
                        ...
                        segs_json.push_back(runLengthEncode(bbox.mask_img,bbox.x1, bbox.y1, bboxwidth, bboxheight,batch_imgs[i].width,batch_imgs[i].height));
                    }
