...
def detect_language(
    ...
    if mel.shape[-2:] != (model.dims.n_audio_ctx, model.dims.n_audio_state):
        ...
        model.encoder_input_tensors_map[0].copy_(fp16_cast(mel))

        start_time = time.time()
        print("detect_language engine.process, graph:", model.encoder_graph_name)
        model.combined_whisper_engine.process(model.encoder_input_tensors_map,
                                              model.encoder_output_tensors_map,
                                              model.stream,
                                              model.encoder_graph_name)
        model.inference_time += time.time() - start_time

        mel_out_tensor = list(model.encoder_output_tensors_map.values())[0]
        mel_out_tensor.to_("host")
        mel_out = torch.from_numpy(uint16_to_fp16(mel_out_tensor.asnumpy()))
        mel_out_tensor.to_("device")
...
class PyTorchInference():
    ...
    def rearrange_kv_cache(
        ...
        if source_indices != list(range(len(source_indices))):
            ...
            self.model.kvcache_rearrange_input_list[0][1].copy_(indices)
            for i in range(2 * self.model.dims.n_text_layer):
                start_time = time.time()
                #import pdb; pdb.set_trace()
                self.model.combined_whisper_engine.process(self.model.kvcache_rearrange_input_list[i], 
                                                           self.model.kvcache_rearrange_output_list[i],
                                                           self.model.stream,
                                                           self.model.kvcache_rearrange_graph_name)
...
class DecodingTask:
    ...
    def _get_audio_features(self, mel: Tensor):
        ...
        if mel.shape[-2:] == (
            ...
            mel = mel.numpy().astype(np.float16)
            mel = mel if mel.flags.c_contiguous else np.ascontiguousarray(mel)
            self.model.encoder_input_tensors_map[0].copy_(fp16_cast(mel));
            start_time = time.time()
            #import pdb; pdb.set_trace()
            self.model.combined_whisper_engine.process(self.model.encoder_input_tensors_map, 
                                                       self.model.encoder_output_tensors_map,
                                                       self.model.stream,
                                                       self.model.encoder_graph_name)
            self.model.inference_time += time.time() - start_time
        ...
        try:
            for i in range(self.sample_len):
                ...
                if i == 0:
                    ...
                    self.model.decoder_main_input_tensors_map[0].copy_(tokens_input)

                    self.model.decoder_main_input_tensors_map[1].copy_(fp16_cast(audio_features))

                    self.model.decoder_main_input_tensors_map[2].copy_(fp16_cast(positional_embedding_input))

                    self.model.decoder_main_input_tensors_map[3].copy_(fp16_cast(mask))

                    start_time = time.time()
                    #import pdb; pdb.set_trace()
                    self.model.combined_whisper_engine.process(self.model.decoder_main_input_tensors_map,
                                                               self.model.decoder_main_output_tensors_map,
                                                               self.model.stream,
                                                               self.model.decoder_main_graph_name)
                    self.model.inference_time += time.time() - start_time

                    x_tensor = self.model.decoder_main_output_tensors_map[0]
                    x_tensor.to_('host')
                    ...
                    self.model.decoder_post_input_tensors_map[0].copy_(fp16_cast(x_sot))

                    self.model.decoder_post_input_tensors_map[1].copy_(fp16_cast(x_last))

                    start_time = time.time()
                    #import pdb; pdb.set_trace()
                    self.model.combined_whisper_engine.process(self.model.decoder_post_input_tensors_map,
                                                               self.model.decoder_post_output_tensors_map,
                                                               self.model.stream,
                                                               self.model.decoder_post_graph_name)
                    self.model.inference_time += time.time() - start_time

                    logits_tensor = self.model.decoder_post_output_tensors_map[0]
                    no_speech_probs_tensor = self.model.decoder_post_output_tensors_map[1]
                    logits_tensor.to_('host')
                    logits = torch.from_numpy(uint16_to_fp16(logits_tensor.asnumpy()))
                    logits_tensor.to_('device')
                    no_speech_probs_tensor.to_('host')
                    no_speech_probs = uint16_to_fp16(no_speech_probs_tensor.asnumpy()).tolist()
                    no_speech_probs_tensor.to_('device')
                    ...
                    # sail
                    self.model.decoder_loop_input_tensors_map[0].copy_(tokens_input)
                    self.model.decoder_loop_input_tensors_map[1].copy_(fp16_cast(positional_embedding_input))
                    self.model.decoder_loop_input_tensors_map[1]

                    self.model.decoder_loop_input_tensors_map[2].copy_(fp16_cast(mask))
                    self.model.decoder_loop_input_tensors_map[2]

                    start_time = time.time()
                    #import pdb; pdb.set_trace()
                    self.model.combined_whisper_engine.process(self.model.decoder_loop_input_tensors_map, 
                                                               self.model.decoder_loop_output_tensors_map,
                                                               self.model.stream,
                                                               self.model.decoder_loop_graph_name)
                    self.model.inference_time += time.time() - start_time

                    logits_tensor = self.model.decoder_loop_output_tensors_map[0]
                    logits_tensor.to_('host')
                    logits = torch.from_numpy(uint16_to_fp16(logits_tensor.asnumpy()))
                    logits_tensor.to_('device')
