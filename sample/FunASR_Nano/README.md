...
- 支持 BM1688 SoC、CV84X6 SoC
- 支持 F16、FP32 模型编译和推理（CV84X6 当前固件不支持 FP32 codegen，仅 F16）
...
bash gen_fp16bmodel_mlir.sh cv84x6  # CV84X6 版本（仅 F16，固件不支持 FP32）
```
...
| SE13-64 | CV84X6/funasr_encoder_f16_1b.bmodel | 43.4 |
| SE13-64 | CV84X6/funasr_adapter_f16_1b.bmodel | 2.4 |

> **测试说明**：
> 1. 性能测试结果具有一定的波动性；
> 2. calculate time 为单次 TPU 推理耗时，不含数据搬运；
> 3. 单 batch 模型无需折算 batch size；
> 4. SE13-64 对应 CV84X6，仅 F16（固件不支持 FP32）。SE13-64 编码器/适配器 TPU 推理可正常工作；
...
> 6. SE13-64(CV84X6) 板端 RAM 仅约 2GB，无法加载完整 FunASR AutoModel（含 Qwen3-0.6B LLM），
