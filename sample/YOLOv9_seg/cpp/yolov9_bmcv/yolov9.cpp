...
#else
	double ratio_w = (double)m_net_w / frame_width;
	double ratio_h = (double)m_net_h / frame_height;
        ImageInfo para = {cv::Size(frame_width, frame_height), {ratio_w, ratio_h}, tx1, ty1}};
#endif
...
#else
            float centerx = ((yolobox_vec[i].x2 + yolobox_vec[i].x1) / 2 - tx1) / ratio_w;
            float centery = ((yolobox_vec[i].y2 + yolobox_vec[i].y1) / 2 - ty1) / ratio_h;
            float width = (yolobox_vec[i].x2 - yolobox_vec[i].x1) / ratio_w;
            float height = (yolobox_vec[i].y2 - yolobox_vec[i].y1) / ratio_h;
#endif
...
void YoloV9::get_mask(const cv::Mat& mask_info,
                      const cv::Mat& mask_data,
                      const ImageInfo& para,
                      cv::Rect bound,
                      cv::Mat& mast_out) {
    cv::Vec<double,2> ratio = para.ratio;
    cv::Vec<int,2> trans = para.trans;
    int r_x = floor((0 * ratio[0] + trans[0]) / 640 * 160);
    int r_y = floor((0 * ratio[1] + trans[1]) / 640 * 160);
    int r_w = ceil(((para.raw_size.width) * ratio[0] + trans[0]) / 640 * 160) - r_x;
    int r_h = ceil(((para.raw_size.height) * ratio[1] + trans[1]) / 640 * 160) - r_y;
    r_w = MAX(r_w, 1);
    r_h = MAX(r_h, 1);
    if (r_x + r_w > 160) {  // crop
...
void YoloV9::clip_boxes(YoloV9BoxVec& yolobox_vec, int src_w, int src_h) {
    for (int i = 0; i < yolobox_vec.size(); i++) {
        yolobox_vec[i].x1 = std::max(0, std::min(yolobox_vec[i].x1, src_w));
        yolobox_vec[i].y1 = std::max(0, std::min(yolobox_vec[i].y1, src_h));
        yolobox_vec[i].x2 = std::max(0, std::min(yolobox_vec[i].x2, src_w));
        yolobox_vec[i].y2 = std::max(0, std::min(yolobox_vec[i].y2, src_h));
    }
}
...
void YoloV9::draw_result(cv::Mat& img, YoloV9BoxVec& result) {
...
void YoloV9::draw_bmcv(bm_handle_t& handle,
    ...
    for (int i = 0; i < result.size(); i++) {
        ...
        cv::Vec<unsigned char, 3> color(colors[result[i].class_id % 25][0],
			                colors[result[i].class_id % 25][1],
                                        colors[result[i].class_id % 25][2]);
        cv::Rect bound = {result[i].x1, result[i].y1, result[i].x2 - result[i].x1, result[i].y2 - result[i].y1};
        bm_image mask_bmimg;
        ...
        bmcv_convert_to_attr_s bmcv_convert_to_attr = {(float)color[0] / 255, 0, (float)color[1] / 255, 0, (float)color[2] / 255, 0};
        ...
        bmcv_rect_t rect = {0, 0, (int)width, (int)height};
    ...
    if (put_text_flag) {
        for (int i = 0; i < result.size(); i++) {
            ...
            left = result[i].x1;
            top = result[i].y1;
            cv::Vec<unsigned char,3> color(colors[result[i].class_id % 25][0], colors[result[i].class_id % 25][1],
                             colors[result[i].class_id % 25][2]);
