...
def main(args):
    ...
    yolov9 = YOLOv9(args)
    batch_size = yolov9.batch_size

    # warm up
    # bmimg = sail.BMImage(yolov9.handle, 1080, 1920, sail.Format.FORMAT_YUV420P, sail.DATA_TYPE_EXT_1N_BYTE)
    # for i in range(10):
    #     results = yolov9([bmimg])
    yolov9.init()
    ...
    if os.path.isdir(args.input):
        ...
        results_list = []
        cn = 0
        yolov9_handle = sail.Handle(args.dev_id)
        for root, dirs, filenames in os.walk(args.input):
            for filename in filenames:
                ...
                bmimg = sail.BMImage()
                ret = decoder.read(yolov9_handle, bmimg)
                ...
                if (len(bmimg_list) == batch_size or cn == len(filenames)) and len(bmimg_list):
                    ...
                    for i, filename in enumerate(filename_list):
                        ...
                        yolov9.postprocess.draw_and_visualize(save_name,bmimg_list[i].asmat(), boxes, segments, vis=False, save=True)
                        res_dict = dict()
    ...
    # test videos
        ...
        while not end_flag:
            frame = sail.BMImage()
            start_time = time.time()
            ret = decoder.read(yolov9_handle, frame)
            decode_time += time.time() - start_time
            ...
            if (len(frame_list) == batch_size or end_flag) and len(frame_list):
                ...
                for i, frame in enumerate(frame_list):
                    ...
                    save_name = os.path.join(output_img_dir, save_basename.replace('.jpg', ''))
                    yolov9.postprocess.draw_and_visualize(save_name,frame_list[i].asmat(), boxes, segments, vis=False, save=True)
                frame_list.clear()
        decoder.release()
    ...
    preprocess_time = yolov9.preprocess_time / cn
    inference_time = yolov9.inference_time / cn
    postprocess_time = yolov9.postprocess_time / cn
...
def argsparser():
    ...
    parser.add_argument('--bmodel', type=str, default='../models/BM1684X/yolov9c_fp32_1b.bmodel', help='path of bmodel')
