...
int main(int argc, char* argv[]) {
    ...
    // get params
    const char* keys =
        "{bmodel_detector | ../../models/BM1684/yolov5s_v6.1_3output_int8_1b.bmodel | detector bmodel file path}"
    ...
    auto bmodel_detector_name = bmodel_detector_name_buffer[bmodel_detector_name_buffer.size() - 1];
    if (info.st_mode & S_IFDIR) {
        ...
        if (stringBuffer[stringBuffer.size() - 1] == "img1")
            save_result_path += stringBuffer[stringBuffer.size() - 2] + "_" + bmodel_detector_name + ".txt";
        else
            save_result_path += stringBuffer[stringBuffer.size() - 1] + "_" + bmodel_detector_name + ".txt";
    } else {
