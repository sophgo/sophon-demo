...
void YoloXDete::enableProfile(TimeStamp *ts)
{
    m_ts = ts;
}
...
void YoloXDete::Inference()
{
    ...
    std::cout << "Inference time use:" << fixed << setprecision(2) << (end_time - start_time) / 1000 << "ms,";
    std::cout << "avg fps:" << fixed << setprecision(1) << batch_size * 1000 * 1000 / (end_time - start_time) << endl;
}
...
VideoProcess::VideoProcess(std::string video_name,
                           ...
                           int resize_width,
                           int resize_height,
                           bm_image_data_format_ext output_dtype) : batch_size(batch_size_),
                                             ...
                                             std::pair<float, float>> &alpha_beta)
{
    ...
    if (batch_size == 1)
    {
        ...
        // bmcv.imwrite("output_image.jpg",output_image);
