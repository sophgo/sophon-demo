...
def getTensors(decoder: sail.Decoder, handle: sail.Handle, bmcv: sail.Bmcv, batch_size, video_w, video_h, resize_w, resize_h, alpha_beta, dtype):
    ...
    if batch_size == 1:
        ...
        output_temp = sail.BMImageArray4D(
            handle, resize_h, resize_w, sail.FORMAT_BGR_PLANAR, dtype)
        for idx in range(4):
    ...
    print("Preprocessed time use:{:.2f} ms, Batch size:{}, avg fps:{:.1f}".format(
        (end_time_pre-start_time_pre)*1000, batch_size, batch_size/(end_time_pre-start_time_pre)))
...
class ByteTracker(object):
    ...
    def inference(self, input_tensor):
        ...
        print("Inference time use:{:.2f} ms, Batch size:{}, avg fps:{:.1f}".format(
             (time.time()-start_time_inf)*1000, batch_size, batch_size/(time.time()-start_time_inf)))
    ...
    def _post_process(self, result, image_info):
        ...
        print("Postprocessed time use:{:.2f} ms, Batch size:{}, avg fps:{:.1f}".format(
             (end_time_post-start_time_post)*1000, batch_size, batch_size/(end_time_post-start_time_post)))
    ...
    def _tracker_update(self, dets, image_info):
        ...
        print("Track time use:{:.2f} ms, Batch size:{}, avg fps:{:.1f}".format(
            (end_time_track-start_time_track)*1000, batch_size, batch_size/(end_time_track-start_time_track)))
...
def get_args():
    ...
    parser.add_argument(
        '--bmodel',
        type=str,
        default='../data/models/BM1684/bytetrack_s_fp32_1b.bmodel',
    )
    parser.add_argument(
        '--file_name',
        type=str,
        # default='../data/video/sample.mp4',
        default='../data/MOT15/ADL-Rundle-6/img1',
    )
    parser.add_argument(
        '--save_path',
        type=str,
...
if __name__ == '__main__':
    ...
    if args.is_video:
        ...
        while True:
            # Pre processed
            _, _, input_tensor, min_ratio = getTensors(decoder, handle, bmcv, batch_size,
                                                       ost_w, ost_h, net_w, net_h, alpha_beta, byte_tracker.dtype)
        ...
        for index in range(0, len(image_list)):
            print("Test No.{} image, total is {}".format(
            ...
            print("Preprocessed time use:{:.2f} ms, Batch size:{}, avg fps:{:.1f}".format(
                (time.time()-start_time_pre)*1000, batch_size, batch_size/(time.time()-start_time_pre)))
    ...
    with open(save_result_name, 'w+') as f:
        for frame_id, tlwhs, track_ids in output_result:
            for tlwh, track_id in zip(tlwhs, track_ids):
                ...
                f.write(line)
    logger.info('Save results to {}'.format(save_result_name))
