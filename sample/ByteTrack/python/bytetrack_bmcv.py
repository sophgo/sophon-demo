...
from yolov5_fuse_bmcv import YOLOv5
...
def update_tracker(target_detector, bytetrack, img_batch, frame_id, mot_saver):
    ...
    for i in range(len(detector_results)):
        ...
        for idx in range(det.shape[0]):
            cls, conf, x1, y1, x2, y2 = det[idx][1:7]
    ...
    for i in range(len(img_batch)):
        cvimg = img_batch[i].asmat()
        detect_num = len(bbox_batch[i])
        outputs = bytetrack._tracker_update(
            bbox_batch[i], confs_batch[i], clss_batch[i], cvimg)
        ...
        image = plot_bboxes(cvimg, bboxes2draw)
...
def main():
    ...
    yolov5_fuse_args = yolov5_fuse_arg(
        args.bmodel_detector, args.dev_id)
    # initialize detector yolov5.
    detector = YOLOv5(yolov5_fuse_args)
    ...
    if os.path.isdir(args.input):  # test image folder
        ...
        # encoder = sail.Encoder("results/video/result.mp4", args.dev_id, 'h264_bm', 'NV12', enc_params, 10)
        fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
        videoWriter = cv2.VideoWriter(
            'results/video/result.mp4', fourcc, 25, (1920, 1080))
        save_id = 0
        # inference flow
        flag = True
        while flag:
