...
## 2. 特性
* 支持BM1688(SoC)、BM1684X(PCIe、SoC)、CV84X6(SoC)
...
├── CV84X6
│   ├── slowfast_cv84x6_fp16_1b.bmodel   # 使用TPU-MLIR编译，用于CV84X6的FP16 BModel，batch_size=1
│   └── slowfast_cv84x6_fp16_4b.bmodel   # 使用TPU-MLIR编译，用于CV84X6的FP16 BModel，batch_size=4
├── BM1688
...
​本例程在`scripts`目录下提供了TPU-MLIR编译FP32 BModel的脚本，请注意修改`gen_fp32bmodel_mlir.sh`中的onnx模型路径、生成模型目录和输入大小shapes等参数，并在执行时指定BModel运行的目标平台（**支持BM1684X/BM1688**；CV84X6当前固件codegen不支持FP32，请改用FP16），如：
...
​本例程在`scripts`目录下提供了量化INT8 BModel的脚本，请注意修改`gen_int8bmodel_mlir.sh`中的onnx模型路径、生成模型目录和输入大小shapes等参数，在执行时输入BModel的目标平台（**支持BM1684X/BM1688**；CV84X6当前工具链INT8模型推理结果错误，暂不支持，请改用FP16），如：
...
| SE13-64      | slowfast_opencv.py | slowfast_cv84x6_fp16_1b.bmodel |    0.632 |
| SE13-64      | slowfast_opencv.py | slowfast_cv84x6_fp16_4b.bmodel |    0.632 |
| SE13-64      | slowfast_opencv.soc | slowfast_cv84x6_fp16_1b.bmodel |    0.557 |
| SE13-64      | slowfast_opencv.soc | slowfast_cv84x6_fp16_4b.bmodel |    0.551 |

> **测试说明**：  
> 1. 由于sdk版本之间可能存在差异，实际运行结果与本表有<0.02的精度误差是正常的；
> 2. 在搭载了相同TPU和SOPHONSDK的PCIe或SoC平台上，相同程序的精度一致，SE7系列对应BM1684X，SE9系列中SE9-16对应BM1688，SE13系列对应CV84X6；
> 3. CV84X6当前固件codegen不支持FP32；当前工具链编译的INT8模型在CV84X6上推理结果错误（与C3D相同的Conv3D量化问题），暂不支持INT8，SE13-64仅测试FP16模型；
...
|   SE13-64   | CV84X6/slowfast_cv84x6_fp16_1b.bmodel        |        141.95  |
|   SE13-64   | CV84X6/slowfast_cv84x6_fp16_4b.bmodel        |        141.13  |
...
|   SE13-64   |slowfast_opencv.py|slowfast_cv84x6_fp16_1b.bmodel|      92.88      |     568.59      |     172.26      |      0.35       |
|   SE13-64   |slowfast_opencv.py|slowfast_cv84x6_fp16_4b.bmodel|      91.46      |     591.74      |     200.33      |      0.18       |
|   SE13-64   |slowfast_opencv.soc|slowfast_cv84x6_fp16_1b.bmodel|      91.00      |     110.59      |     140.34      |      0.51       |
|   SE13-64   |slowfast_opencv.soc|slowfast_cv84x6_fp16_4b.bmodel|      79.15      |     109.85      |     138.30      |      0.48       |
