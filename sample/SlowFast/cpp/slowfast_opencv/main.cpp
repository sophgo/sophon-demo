...
int main(int argc, char** argv) {
     ...
     */
    const char* keys =
        "{bmodel | ../../models/BM1684X/slowfast_bm1684x_fp32_1b.bmodel | bmodel file path}"
        "{dev_id | 0 | TPU device id}"
        "{help | 0 | print help information.}"
        "{input | ../../datasets/test | sampled kinetics400 video directory}"
        "{classnames |  ../../datasets/kinetics_classnames.txt | kinetics400 class names}";
    ...
    std::string kinetics_names = parser.get<std::string>("classnames");
    if (stat(kinetics_names.c_str(), &info) != 0) {
    ...
    SlowFast slowfast(bm_ctx, step_len, dev_id);
    slowfast.Init();
    int batch_size = slowfast.batch_size();

    // 3. Profile and save results
    TimeStamp slowfast_ts;
    TimeStamp* ts = &slowfast_ts;
    slowfast.enableProfile(&slowfast_ts);
    ...
    std::ifstream ifs(kinetics_names);
    if (ifs.is_open()) {
        std::string line;
    ...
    for (int j = 0; j < video_paths.size(); j++) {
        ...
        if (batch_videos.size() == batch_size) {
            ...
            slowfast.detect(batch_videos, predict_ids);
    ...
    ts->build_timeline("SlowFast detect");
    ts->show_summary("SlowFast detect");
    ts->clear();
    return 0;
}
