...
* 图像压缩(embedding)部分支持FP16 1batch、INT8 1batch(BM1684X)；CV84X6支持FP16 1batch
* 图像推理(mask_decoder)部分支持FP32 1batch、FP16 1batch(BM1684X)；CV84X6支持FP16 1batch
* 支持基于OpenCV的Python推理
...
**注意，CV84X6当前固件codegen不支持FP32（含matmul的decoder编译时tpuc-opt断言失败），CV84X6请使用FP16 BModel。**
...
CV84X6平台请使用`./scripts/gen_fp16bmodel_mlir.sh cv84x6`。
...
**注意，CV84X6暂不支持INT8 embedding bmodel：量化校准时TPU-MLIR解释器需同时驻留ViT-B全部中间激活（峰值约28GB主机内存），常规环境会OOM被杀。CV84X6请使用FP16 embedding bmodel。**
...
**注意，CV84X6当前固件不支持FP32，无法编译auto multi decoder bmodel，自动掩码生成（--auto 1）暂不支持在CV84X6上使用。**
...
> 3. SoC和PCIe的测试结果基本一致；
> 4. SE13系列为CV84X6平台，目前仅提供fp16 bmodel（fp32/int8编译受限，见第4节说明）。
