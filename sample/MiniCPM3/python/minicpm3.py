...
def argsparser():
    ...
    parser.add_argument('--config', type=str, default='./config/minicpm3.yaml', help='path of config file')
...
if __name__ == "__main__":
    ...
    minicpm = MiniCPM(config["bmodel_path"], config["dev_ids"], config["token_path"])
    messages = []
    while True:
        input_str = input("\nQuestion: ")
        if input_str == "exit":
