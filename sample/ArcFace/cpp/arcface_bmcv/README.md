...
| 序号  | C++例程      | 说明                                 |
| ---- | ------------- | -----------------------------------  |
| 1    | arcface_bmcv  | 使用SOPHON-OPENCV解码、BMCV前处理、BMRT推理   |
...
**注：** 如果直接在SoC设备上编译（如SE7-32已预装gcc和所需库），也可以使用以下命令直接编译：

```bash
cd cpp/arcface_bmcv
g++ -O3 -o arcface_bmcv.soc arcface.cpp main.cpp \
  -I/opt/sophon/libsophon-current/include \
  -I/opt/sophon/sophon-opencv-latest/include/opencv4 \
  -I/opt/sophon/sophon-ffmpeg-latest/include \
  -L/opt/sophon/libsophon-current/lib \
  -L/opt/sophon/sophon-opencv-latest/lib \
  -L/opt/sophon/sophon-ffmpeg-latest/lib \
  -lbmrt -lbmlib -lbmcv -lopencv_imgproc -lopencv_core \
  -lopencv_highgui -lopencv_imgcodecs -lpthread -ldl
```
...
```
测试结束后，会打印每张图片的L2归一化嵌入向量、各阶段耗时和性能汇总信息。FP32/FP16模型与Python SAIL版本产生完全一致的结果。
