...
if [ ! -d "../models/bert-base-uncased" ]; 
then
    python3 -m dfss --url=open@sophgo.com:sophon-demo/GroundingDINO/models/bert-base-uncased.tar.gz
    tar xvf bert-base-uncased.tar.gz && rm bert-base-uncased.tar.gz
    echo "models/bert-base-uncased download!"
else
    echo "models/bert-base-uncased folder exist! Remove it if you need to update."
fi

if [ ! -d "../models/BM1684X" ]; 
then
    if [ $download_bm1684x == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/GroundingDINO/models/BM1684X.tar.gz
...
then
    if [ $download_bm1684x2 == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/GroundingDINO/models/BM1684X2.tar.gz
...
then
    if [ $download_bm1688 == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/GroundingDINO/models/BM1688.tar.gz
...
then
    if [ $download_cv186x == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/GroundingDINO/models/CV186X.tar.gz
...
then
    if [ $download_onnx == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/GroundingDINO/models/onnx.tar.gz
