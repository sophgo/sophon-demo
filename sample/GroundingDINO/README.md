...
## 2. 特性
* 支持BM1684X(x86 PCIe、SoC)，CV84X6(SE13-64 SoC)，BM1688(SoC)、CV186X(SoC)
* 支持FP16模型编译和推理
* 支持基于PIL的Python推理
...
│   ├── CV84X6
│   │  └── groundingdino_cv84x6_fp16.bmodel  # 使用TPU-MLIR编译，用于CV84X6的FP16 BModel，batch_size=1
│   ├── torch
...
# 针对cv84x6平台(SE13-64)
./scripts/gen_fp16bmodel_mlir.sh cv84x6   # 注意：CV84X6版本脚本内含TopK use_hau workaround，见下方说明
# 针对cv186x平台
...
> **CV84X6(SE13-64) 编译说明**：  
> CV84X6 上 `tpu.TopK`(`use_hau=true`，即 HAU 动态子网路径) 会使固件挂死 TPU（bmrt_test 卡住并触发 driver `wait api timeout`）。
> `gen_fp16bmodel_mlir.sh cv84x6` 已内置规避：先正常跑到 stage-1，再用 `sed` 将 TopK 的 `use_hau` 改为 `false`（走静态多核路径），然后手动执行 stage-2 + codegen。
> 使用 `--bmodel` 时无需任何额外操作，直接用脚本生成的 `groundingdino_cv84x6_fp16.bmodel` 即可。
...
|   SE13-64   | groundingdino_cv84x6_fp16.bmodel  | 0.507                  |
...
| SE13-64     | groundingdino_pil.py | groundingdino_cv84x6_fp16.bmodel | 8.33        | 183.45           | 519.68          | 12.77             |
...
| SE13-64     | groundingdino_sail.soc | groundingdino_cv84x6_fp16.bmodel | 8.15     | 15.49           | 505.19          | 6.16              |
