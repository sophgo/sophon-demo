...
class BlockDiagonalLinear(nn.Module):
  ...
  def forward(self, x):
    ...
    # Original: torch.einsum("... h i, h i j -> ... h j", x, self.w).
    # Rewritten to batched matmul because TPU-MLIR's top.Einsum does not support
    # the ellipsis ("...") form. For each block h: y[h] = x[h] @ w[h].
    H = self.num_blocks
    I = x.shape[-1]
    J = self.w.shape[-1]
    x_3d = x.reshape(-1, H, I)                                     # [N, H, I]
    y = torch.bmm(x_3d.transpose(0, 1), self.w).transpose(0, 1)    # [N, H, J]
    y = y.reshape(*x.shape[:-1], J)                                # [..., H, J]
    y = y + self.b
...
class RGLRU(nn.Module):
  ...
  def forward(self, x, cache=None, use_linear_scan=True):
    ...
    # `sqrt` in order to prevent NaNs during training in bfloat16. In inference /
    # export (eval, no_grad) the forward of SqrtBoundDerivative is plain sqrt, so
    # use torch.sqrt directly to keep the ONNX graph op-pure (no custom op).
...
class Einsum(nn.Module):
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    Specialized for the only equation used in TAPNext++: ``...td,cdD->c...tD``
    (MLPBlock.ffw_up, w_shape=(2,width,expanded_width)). Rewritten to matmul
    because TPU-MLIR's top.Einsum does not support the ellipsis form.
    out[c, ..., t, D] = sum_d x[..., t, d] * w[c, d, D], i.e. out[c] = x @ w[c].
    The ``...`` captures the token (and any batch) dims, so a single batched
    matmul would mis-broadcast c against the token dim; instead unroll over
    c (=2, GLU gating) into two plain matmuls + a stack. ONNX exports this as
    2 MatMul + Concat, all TPU-MLIR-supported.
    """
    outs = [torch.matmul(x, self.w[c]) for c in range(self.w.shape[0])]
    return torch.stack(outs, dim=0) + self.b
