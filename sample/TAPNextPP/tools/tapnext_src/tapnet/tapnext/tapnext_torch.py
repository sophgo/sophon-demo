...
# torch.export / pytree registration is only needed for torch.export & FX
# tracing (torch>=2.0). For ONNX export (torch.onnx.export) it is a no-op, so
# guard it to stay importable under torch 1.13 (used for clean MHA export).
try:
    torch.export.register_dataclass(TAPNextTrackingState)
except Exception:  # pragma: no cover  (torch<2.0 has no torch.export)
    pass
