...
int YoloV8_seg::Detect(const std::vector<bm_image>& input_images, std::vector<YoloV8BoxVec>& boxes) {
...
float YoloV8_seg::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int YoloV8_seg::pre_process(const std::vector<bm_image>& images, 
                            bm_tensor_t& input_tensor, 
                            std::vector<std::pair<int, int>>& txy_batch, 
                            std::vector<std::pair<float, float>>& ratios_batch) {
    int ret = 0;
 ...
 */
float* YoloV8_seg::get_cpu_data(bm_tensor_t* tensor, float scale){
    int ret = 0;
    float *pFP32 = NULL;
...
int YoloV8_seg::post_process(const std::vector<bm_image>& input_images, 
                             std::vector<bm_tensor_t>& output_tensors, 
...
int YoloV8_seg::argmax(float* data, int num) {
    float max_value = 0.0;
    int max_index = 0;
    for (int i = 0; i < num; ++i) {
        float value = data[i];
...
void YoloV8_seg::get_mask(const cv::Mat& mask_info,
...
void YoloV8_seg::xywh2xyxy(YoloV8BoxVec& xyxyboxes, std::vector<std::vector<float>> box) {
...
void YoloV8_seg::NMS(YoloV8BoxVec& dets, float nmsConfidence) {
...
void YoloV8_seg::draw_result(cv::Mat& img, YoloV8BoxVec& result) {
