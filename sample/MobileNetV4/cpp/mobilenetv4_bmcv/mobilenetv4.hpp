...
#ifndef MOBILENETV4_H
#define MOBILENETV4_H
...
public:
    int batch_size = -1;
    TimeStamp* m_ts = NULL;
    MOBILENETV4(std::string bmodel_file, int dev_id = 0){
        // get handle
        ...
        // MobileNetV4 preprocessing: mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
        // formula: (img/255 - mean) / std * input_scale
        // converto: alpha = 1/(255*std) * input_scale, beta = (-mean/std) * input_scale
        float input_scale = netinfo->input_scales[0];
        converto_attr.alpha_0 = 1 / (255. * 0.229) * input_scale;
        converto_attr.beta_0 = (-0.485 / 0.229) * input_scale;
        converto_attr.alpha_1 = 1 / (255. * 0.224) * input_scale;
        converto_attr.beta_1 = (-0.456 / 0.224) * input_scale;
        converto_attr.alpha_2 = 1 / (255. * 0.225) * input_scale;
        converto_attr.beta_2 = (-0.406 / 0.225) * input_scale;
    ...
    int Classify(const std::vector<bm_image>& images, std::vector<std::pair<int, float>>& results);
};
