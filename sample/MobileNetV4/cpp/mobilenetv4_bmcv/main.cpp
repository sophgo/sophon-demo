...
int main(int argc, char *argv[]) {
  ...
  const char *keys="{input | ../../datasets/imagenet_val_1k/img | input path, image file path}"
    "{bmodel | ../../models/BM1684X/mobilenetv4_conv_medium_fp32_1b.bmodel | bmodel file path}"
  ...
  // initialize net
  MOBILENETV4 mobilenetv4(bmodel_file, dev_id);
  cout << "set device id: " << dev_id << endl;

  // profiling
  TimeStamp mobilenetv4_ts;
  TimeStamp *ts = &mobilenetv4_ts;
  mobilenetv4.m_ts = ts;

  // get batch_size
  int batch_size = mobilenetv4.batch_size;
  ...
  if (info.st_mode & S_IFREG) {
    ...
    ts->save("mobilenetv4 overall");
    ts->save("decode time");
    // decode jpg using opencv
    cv::Mat mat = cv::imread(input, cv::IMREAD_COLOR, dev_id);
    if (mat.empty()) {
    ...
    // do infer
    CV_Assert(0 == mobilenetv4.Classify(batch_imgs, results));
    ts->save("mobilenetv4 overall");
    ...
    // Sort files for deterministic order (matching YOLOv8_plus_cls)
    ...
    for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++) {
      ...
      // Keep cv::Mat alive — bmimg may reference its memory
      batch_mats.push_back(mat);
      batch_imgs.push_back(bmimg);
  ...
  mobilenetv4_ts.calbr_basetime(base_time);
  mobilenetv4_ts.build_timeline("mobilenetv4 infer");
  mobilenetv4_ts.show_summary("mobilenetv4 infer");
  mobilenetv4_ts.clear();
