...
def main(args):
    mobilenet = MobileNetV4(args)
    batch_size = mobilenet.batch_size
    ...
    for filename in glob.glob(args.input+'/*'):
        ...
        bmimg = sail.BMImage()
        ret = decoder.read(mobilenet.handle, bmimg)
        if ret != 0:
    ...
    preprocess_time = mobilenet.preprocess_time / cn
    inference_time = mobilenet.inference_time / cn
    postprocess_time = mobilenet.postprocess_time / cn
...
def argsparser():
    ...
    parser.add_argument('--bmodel', type=str, default='../models/BM1684X/mobilenetv4_conv_medium_fp32_1b.bmodel', help='path of bmodel')
