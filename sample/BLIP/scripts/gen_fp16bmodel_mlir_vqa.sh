...
function gen_fp16bmodel()
{
    model_deploy.py \
        ...
        --quantize F16 \
        --chip ${target} \
        --model ${name}_venc_${target}_f16_$1b.bmodel
    mv ./${name}_venc_${target}_f16_$1b.bmodel $outdir

    model_deploy.py \
        --mlir ${name}_tenc_$1b.mlir \
        --quantize F16 \
        --chip ${target} \
        --model ${name}_tenc_${target}_f16_$1b.bmodel
    mv ./${name}_tenc_${target}_f16_$1b.bmodel $outdir

    model_deploy.py \
        --mlir ${name}_tdec_$1b.mlir \
        --quantize F16 \
        --chip ${target} \
        --model ${name}_tdec_${target}_f16_$1b.bmodel
    mv ./${name}_tdec_${target}_f16_$1b.bmodel $outdir
}
