...
int main(int argc, char* argv[]) {
    ...
    // get params
    const char* keys =
        "{bmodel | ../../models/BM1684X/yolov5s_tpukernel_fp32_1b.bmodel | bmodel file path}"
    ...
    if (stat(tpu_kernel_module_path.c_str(), &info) != 0) {
        cout << "Cannot find tpu_kernel_module_path file." << endl;
        exit(1);
    }
    ...
    CV_Assert(0 == yolov5.Init(parser.get<float>("conf_thresh"), parser.get<float>("nms_thresh"), tpu_kernel_module_path, coco_names));
