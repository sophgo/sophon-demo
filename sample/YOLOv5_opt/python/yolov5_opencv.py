...
class YOLOv5:
    def __init__(self, args):
        ...
        if len(self.output_names) not in [3]:
            raise ValueError('only support 3 outputs, but got {} outputs bmodel'.format(len(self.output_names)))
        self.output_shapes = []
        self.output_tensors = {}
        for output_name in self.output_names:
            output_shape = self.net.get_output_shape(self.graph_name, output_name)
            if len(output_shape) != 4:
                raise ValueError('only support outputs with 4 dimension, but got {} dimension'.format(len(output_shape)))
            output_dtype = self.net.get_output_dtype(self.graph_name, output_name)
            self.output_shapes.append(output_shape)
            self.output_tensors[output_name] = sail.Tensor(self.net.get_handle(), output_shape, output_dtype, True, True)
        self.batch_size = self.input_shape[0]
        self.net_h = self.input_shape[2]
        self.net_w = self.input_shape[3]
        
        self.conf_thresh = max(0.1, args.conf_thresh)
        self.nms_thresh = max(0.1, args.nms_thresh)
        self.agnostic = False
        self.multi_label = True
        self.max_det = 1000
        
        #tpu_kernel_post_process, use default anchors, you can reset anchors using sail api.
        self.tpu_kernel_post = sail.tpu_kernel_api_yolov5_detect_out(0, self.output_shapes, self.net_w, self.net_h, args.tpu_kernel_module_path)
    ...
    def predict(self, input_img, img_num):
        input_tensor = sail.Tensor(self.net.get_handle(), input_img, False)
        ...
        # resort
        out_keys = list(self.output_tensors.keys())
        ord = []
        for n in self.output_names:
            for i, k in enumerate(out_keys):
                if n == k:
                    ord.append(i)
                    break
        out = [self.output_tensors[out_keys[i]].asnumpy()[:img_num] for i in ord]
        return out
    ...
    def __call__(self, img_list):
        ...
        start_time = time.time()
        #results:[x1,y1,w,h,class_id,score]
        result_list = self.tpu_kernel_post.process(self.output_tensors, self.conf_thresh, self.nms_thresh)
        results = []
...
def main(args):
    ...
    if not os.path.exists(args.tpu_kernel_module_path):
        raise FileNotFoundError('{} is not existed.'.format(args.tpu_kernel_module_path))
...
def argsparser():
    ...
    parser.add_argument('--bmodel', type=str, default='../models/BM1684X/yolov5s_tpukernel_int8_1b.bmodel', help='path of bmodel')
    ...
    parser.add_argument('--tpu_kernel_module_path', type=str, default="../tpu_kernel_module/libbm1684x_kernel_module.so", help='tpu_kernel_module_path')
