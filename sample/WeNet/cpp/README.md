...
CV84X6平台（如SE13-64）上，请使用FP16或INT8模型（当前固件codegen不支持FP32），可执行文件为`wenet.soc`，运行时需要设置`LD_LIBRARY_PATH`指向`cpp/cross_compile_module`下的运行库（由[准备模型与数据](../README.md#3-准备模型与数据)下载的3rd_party和ctcdecode提供，SoC环境）：
```bash
...
./wenet.soc --encoder_bmodel=../models/CV84X6/wenet_encoder_streaming_fp16.bmodel --dict_file=../config/lang_char.txt --config_file=../config/train_u2++_conformer.yaml --result_file=./result.txt --input=../datasets/aishell_S0764/aishell_S0764.list --mode=ctc_prefix_beam_search --dev_id=0
./wenet.soc --encoder_bmodel=../models/CV84X6/wenet_encoder_streaming_int8.bmodel --decoder_bmodel=../models/CV84X6/wenet_decoder_int8.bmodel --dict_file=../config/lang_char.txt --config_file=../config/train_u2++_conformer.yaml --result_file=./result.txt --input=../datasets/aishell_S0764/aishell_S0764.list --mode=attention_rescoring --dev_id=0
```  
