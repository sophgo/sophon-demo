...
int Yolo26_obb::Detect(const std::vector<bm_image>& input_images, std::vector<obbBoxVec_xyxy>& boxes) {
...
float Yolo26_obb::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int Yolo26_obb::pre_process(const std::vector<bm_image>& images, 
...
int Yolo26_obb::forward(bm_tensor_t& input_tensor, bm_tensor_t& output_tensor){
    // static int count = 0;
 ...
 */
float* Yolo26_obb::get_cpu_data(bm_tensor_t* tensor, float scale){
    int ret = 0;
    float *pFP32 = NULL;
...
int Yolo26_obb::post_process(const std::vector<bm_image>& input_images, 
    ...
    for (int batch_idx = 0; batch_idx < input_images.size(); ++batch_idx) {
        ...
        output_data = tensor_data + batch_idx * box_num * nout; //output_tensor: [bs, box_num, 7]

        // Candidates
        LOG_TS(m_ts, "yolo26_obb_post: get box");
        // float* cls_conf = output_data + 4; //output_tensor's last dim: [x, y, w, h, cls_conf, cls_id, rotate_angle]
        for (int i = 0; i < box_num; i++) {
            // best class
            obbBox box;
            box.score = output_data[i * nout + 4];
            box.class_id = output_data[i * nout + 5];
            if(box.score <= m_confThreshold){
                continue;
            }
            box.x = output_data[i * nout + 0];
            box.y = output_data[i * nout + 1];
            box.w = output_data[i * nout + 2];
            box.h = output_data[i * nout + 3];
            box.angle = output_data[i * nout + 6];
...
void Yolo26_obb::regularize_rbox(obbBoxVec& obbVec){
    for(auto& obb : obbVec){
        if(obb.h > obb.w){
...
obbBoxVec_xyxy Yolo26_obb::xywhr2xyxyxyxy(const obbBoxVec& obbVec){
...
void Yolo26_obb::drawPred(obbBoxVec_xyxy& box_vec, cv::Mat& frame, float draw_thresh)   // Draw the predicted bounding box
{
