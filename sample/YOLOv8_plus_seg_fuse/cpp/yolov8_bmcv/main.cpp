...
int main(int argc, char* argv[]) {
    ...
    // get params
    const char* keys =
        "{bmodel | ../../models/BM1684X/yolov8s_seg_fuse_int8_1b.bmodel | bmodel file path}"
        "{dev_id | 0 | TPU device id}"
        "{draw_thresh | 0.5 | draw box thresh}"
    ...
    // initialize net
    YoloV8SegFuse yolov8(bmodel_file, coco_names, dev_id);
        ...
        for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++) {
            ...
            if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
                ...
                for (int i = 0; i < batch_imgs.size(); i++) {
                    yolov8.draw_result(batch_mats[i], boxes[i], draw_thresh);
