...
int main(int argc, char *argv[]){
    ...
    // initialize net
    YoLoV11 yolov11(bm_ctx);
    CV_Assert(0 == yolov11.Init(
        parser.get<float>("conf_thresh"),
        parser.get<float>("nms_thresh"),
        coco_names));

      // profiling
    TimeStamp yolov11_ts;
    TimeStamp *ts = &yolov11_ts;
    yolov11.enableProfile(&yolov11_ts);

    // get batch_size
    int batch_size = yolov11.batch_size();
    ...
    if (info.st_mode & S_IFDIR){
        ...
        for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++){
            ...
            if ((int)batch_imgs.size() == batch_size){
                // predict
                CV_Assert(0 == yolov11.Detect(batch_imgs, boxes));
                ...
                for(int i = 0; i < batch_size; i++){
                    ...
                    if(boxes[i].size() != 0){
                        for (auto bbox : boxes[i]) {
                            ...
                            // draw image
                            if(bbox.score > 0.25)
                                yolov11.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x1, bbox.y1, bboxwidth, bboxheight, batch_imgs[i]);
                            // save result
                            json bbox_json;
    ...
    yolov11_ts.calbr_basetime(base_time);
    yolov11_ts.build_timeline("yolov11 test");
    yolov11_ts.show_summary("yolov11 test");
    yolov11_ts.clear();
    return 0;
}