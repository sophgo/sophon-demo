...
YoLoV11::YoLoV11(std::shared_ptr<BMNNContext> context) : m_bmContext(context) {
    std::cout << "YoLoV11 ctor .." << std::endl;
}

YoLoV11::~YoLoV11() {
    std::cout << "YoLoV11 dtor ..." << std::endl;
...
int YoLoV11::Init(float confThresh, float nmsThresh, const std::string& coco_names_file) {
...
void YoLoV11::enableProfile(TimeStamp* ts) {
    m_ts = ts;
}

int YoLoV11::batch_size() {
    return max_batch;
};

int YoLoV11::Detect(const std::vector<bm_image>& input_images, std::vector<YoLoV11BoxVec>& boxes) {
    int ret = 0;
    // 3. preprocess
    m_ts->save("yolov11 preprocess", input_images.size());
    ret = pre_process(input_images);
    CV_Assert(ret == 0);
    m_ts->save("yolov11 preprocess", input_images.size());

    // 4. forward
    m_ts->save("yolov11 inference", input_images.size());
    ret = m_bmNetwork->forward();
    CV_Assert(ret == 0);
    m_ts->save("yolov11 inference", input_images.size());

    // 5. post process
    m_ts->save("yolov11 postprocess", input_images.size());
    if(use_cpu_opt){
#if USE_OPT
...
int YoLoV11::pre_process(const std::vector<bm_image>& images) {
...
float YoLoV11::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int YoLoV11::post_process(const std::vector<bm_image>& images, std::vector<YoLoV11BoxVec>& detected_boxes) {
    YoLoV11BoxVec yolobox_vec;
...
#if USE_OPT
int YoLoV11::post_process_opt(const std::vector<bm_image>& images, std::vector<YoLoV11BoxVec>& detected_boxes) {
    YoLoV11BoxVec yolobox_vec;
    std::vector<cv::Rect> bbox_vec;
    std::vector<std::shared_ptr<BMNNTensor>> outputTensors(output_num);
...
void YoLoV11::clip_boxes(YoLoV11BoxVec& yolobox_vec, int src_w, int src_h) {
    for (int i = 0; i < yolobox_vec.size(); i++) {
...
void YoLoV11::xywh2xyxy(YoLoV11BoxVec& xyxyboxes, std::vector<std::vector<float>> box) {
...
void YoLoV11::NMS(YoLoV11BoxVec& dets, float nmsConfidence) {
    int length = dets.size();
    int index = length - 1;

    std::sort(dets.begin(), dets.end(), [](const YoLoV11Box& a, const YoLoV11Box& b) { return a.score < b.score; });
...
void YoLoV11::draw_bmcv(bm_handle_t& handle,
                       int classId,
                       float conf,
                       int left,
                       int top,
