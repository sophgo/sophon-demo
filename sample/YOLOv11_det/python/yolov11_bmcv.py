...
def main(args):
    ...
    yolov11 = YOLOv11(args)
    batch_size = yolov11.batch_size
    
    # warm up 
    # bmimg = sail.BMImage(yolov11.handle, 1080, 1920, sail.Format.FORMAT_YUV420P, sail.DATA_TYPE_EXT_1N_BYTE)
    # for i in range(10):
    #     results = yolov11([bmimg])
    yolov11.init()
    ...
    if os.path.isdir(args.input): 
        ...
        results_list = []
        cn = 0
        yolov11_handle = sail.Handle(args.dev_id)
        for root, dirs, filenames in os.walk(args.input):
            for filename in filenames:
                ...
                bmimg = sail.BMImage()
                ret = decoder.read(yolov11_handle, bmimg)
                if ret != 0:
    ...
    # test videos
        ...
        while True:
            frame = sail.BMImage()
            start_time = time.time()
            ret = decoder.read(yolov11_handle, frame)
            if ret:
                break
            decode_time += time.time() - start_time
            frame_list.append(frame)
            if len(frame_list) == batch_size:
                results = yolov11(frame_list)
    ...
    preprocess_time = yolov11.preprocess_time / cn
    inference_time = yolov11.inference_time / cn
    postprocess_time = yolov11.postprocess_time / cn
...
def argsparser():
    ...
    parser.add_argument('--bmodel', type=str, default='../models/BM1684X/yolov11s_fp32_1b.bmodel', help='path of bmodel')
