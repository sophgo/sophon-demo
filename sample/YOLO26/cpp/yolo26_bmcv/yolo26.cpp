...
Yolo26::Yolo26(std::shared_ptr<BMNNContext> context) : m_bmContext(context) {
    std::cout << "Yolo26 ctor .." << std::endl;
}

Yolo26::~Yolo26() {
    std::cout << "Yolo26 dtor ..." << std::endl;
...
int Yolo26::Init(float confThresh, const std::string& coco_names_file) {
...
void Yolo26::enableProfile(TimeStamp* ts) {
    m_ts = ts;
}

int Yolo26::batch_size() {
    return max_batch;
};

int Yolo26::Detect(const std::vector<bm_image>& input_images, std::vector<Yolo26BoxVec>& boxes) {
    int ret = 0;
    // 3. preprocess
...
int Yolo26::pre_process(const std::vector<bm_image>& images) {
...
float Yolo26::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int Yolo26::post_process(const std::vector<bm_image>& images, std::vector<Yolo26BoxVec>& detected_boxes) {
...
void Yolo26::clip_boxes(Yolo26BoxVec& yolobox_vec, int src_w, int src_h) {
    for (int i = 0; i < yolobox_vec.size(); i++) {
...
void Yolo26::draw_bmcv(bm_handle_t& handle,
                       int classId,
                       float conf,
                       int left,
                       int top,
