...
int main(int argc, char *argv[]) {
    cout.setf(ios::fixed);
    // get params
    const char *keys = "{bmodel | ../../models/BM1684X/yolo26s_fp32_1b.bmodel | bmodel file path}"
    ...
    // initialize net
    Yolo26 yolo26(bm_ctx);
    CV_Assert(0 == yolo26.Init(parser.get<float>("conf_thresh"), coco_names));

    // profiling
    TimeStamp yolo26_ts;
    TimeStamp *ts = &yolo26_ts;
    yolo26.enableProfile(&yolo26_ts);
    ...
    if (info.st_mode & S_IFDIR) {
        ...
        for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++) {
            ...
            if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
                // predict
                CV_Assert(0 == yolo26.Detect(batch_imgs, boxes));
                ...
                for (int i = 0; i < batch_imgs.size(); i++) {
                    ...
                    for (auto bbox : boxes[i]) {
                        ...
                        // draw image
#if DEBUG
                        cout << "  class id=" << bbox.class_id << ", score = " << bbox.score << " (x1=" << bbox.x1
                             << ",y1=" << bbox.y1 << ",w=" << bboxwidth << ",h=" << bboxheight << ")" << endl;
#endif
                        yolo26.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x1, bbox.y1, bboxwidth, bboxheight,
                                         batch_imgs[i]);
                        json bbox_json;
...
// save image
    ...
    yolo26_ts.build_timeline("yolo26 test");
    yolo26_ts.show_summary("yolo26 test");
    yolo26_ts.clear();
    return 0;
}