...
## 1. Introduction
YOLOv5 is a very classical One Stage target detection algorithm based on anchor. Because of its excellent accuracy and speed performance, it has been widely used in engineering practice. This example [​YOLOv5 official open source repository](https://github.com/ultralytics/yolov5) transplants the v6.1 version of the model and algorithm so that it can be inference tested on SOPHON BM1684/BM1684X/CV84X6/BM1688/CV186X.
...
* Support for BM1688/CV186X(SoC), BM1684X(x86 PCIe、SoC、riscv PCIe), CV84X6(SoC), BM1684(x86 PCIe、SoC、arm PCIe)
* Support for FP32, FP16 (BM1684X/CV84X6/BM1688/CV186X), INT8 model compilation and inference
...
├── CV84X6
│   ├── yolov5s_v6.1_3output_fp32_1b.bmodel       # Compiled with TPU-MLIR, FP32 BModel,batch_size=1 for CV84X6
│   ├── yolov5s_v6.1_3output_fp16_1b.bmodel       # Compiled with TPU-MLIR, FP16 BModel,batch_size=1 for CV84X6
│   ├── yolov5s_v6.1_3output_int8_1b.bmodel       # Compiled with TPU-MLIR, INT8 BModel,batch_size=1 for CV84X6
│   └── yolov5s_v6.1_3output_int8_4b.bmodel       # Compiled with TPU-MLIR, INT8 BModel,batch_size=4 for CV84X6
├── BM1688
...
> 4. On a PCIe or SoC platform equipped with the same TPU and SOPHONSDK, the mAP of the same program is the same, SE5 series corresponds to BM1684, SE7 series corresponds to BM1684X. In SE9 series, SE9-16 corresponds to BM1688, SE9-8 corresponds to CV186X, SE13 series corresponds to CV84X6;
> 5. The hardware JPEG decoder (`/dev/soph_vc_dec`) of SE13-64 (CV84X6) has a defect in the current firmware (libsophon 0.4.13). The examples that use hardware decoding (`yolov5_bmcv.py`, `yolov5_bmcv.soc`) crash with memory corruption (`malloc(): invalid size` / segfault) when processing images and cannot complete the test, so this table only gives the results of software decoding (`yolov5_opencv.py`); `yolov5_opencv.py` (software decoding) can represent the real inference precision of the CV84X6 TPU. This issue has been reported to the multimedia firmware team, and the hardware decoding path should work normally after the firmware is fixed.
...
| CV84X6/yolov5s_v6.1_3output_fp32_1b.bmodel|          62.03  |
| CV84X6/yolov5s_v6.1_3output_fp16_1b.bmodel|           8.36  |
| CV84X6/yolov5s_v6.1_3output_int8_1b.bmodel|           3.89  |
| CV84X6/yolov5s_v6.1_3output_int8_4b.bmodel|          14.26  |
...
|   SE9-16    |  yolov5_sail.soc  |yolov5s_v6.1_3output_int8_4b_2core.bmodel|      3.36       |      4.55       |      7.26       |      19.77      |
