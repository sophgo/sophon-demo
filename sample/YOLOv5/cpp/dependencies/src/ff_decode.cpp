...
bm_status_t avframe_to_bm_image(bm_handle_t& handle, AVFrame* in, bm_image* out, bool is_jpeg, bool data_on_device_mem, int coded_width, int coded_height) {
    ...
    if (in->channel_layout == 101) { /* COMPRESSED NV12 FORMAT */
        ...
        auto ret = bm_image_alloc_dev_mem(*out);
        ...
        if (data_on_device_mem) {
            ...
            void *addr;
            auto tpuret = tpuRtMalloc(&addr, size, 0);
            input_addr[0] = bm_mem_from_device(reinterpret_cast<unsigned long long>(addr), size);
            assert(tpuret == tpuRtSuccess);
            tpuret = tpuRtMemcpyS2D(addr, in->data[0], size);
            assert(tpuret == tpuRtSuccess);
        }
        ...
        if (data_five_denominator != -1) {
            ...
            if (data_on_device_mem) {
                ...
                void *addr;
                auto tpuret = tpuRtMalloc(&addr, size, 0);
                input_addr[1] = bm_mem_from_device(reinterpret_cast<unsigned long long>(addr), size);
                assert(tpuret == tpuRtSuccess);
                tpuret = tpuRtMemcpyS2D(addr, in->data[1], size);
                assert(tpuret == tpuRtSuccess);
            }
        }
        ...
        if (data_six_denominator != -1) {
            ...
            if (data_on_device_mem) {
                ...
                void *addr;
                auto tpuret = tpuRtMalloc(&addr, size, 0);
                input_addr[2] = bm_mem_from_device(reinterpret_cast<unsigned long long>(addr), size);
                assert(tpuret == tpuRtSuccess);
                tpuret = tpuRtMemcpyS2D(addr, in->data[2], size);
                assert(tpuret == tpuRtSuccess);
            }
        }
...
int VideoDecFFM::openDec(bm_handle_t dec_handle, const char* input) {
    ...
    ret = openCodecContext(&video_stream_idx, &video_dec_ctx, ifmt_ctx, AVMEDIA_TYPE_VIDEO, handle->dev_id);
...
void* VideoDecFFM::vidPushImage() {
    while (1) {
        ...
        avframe_to_bm_image(handle, avframe, img, false, this->data_on_device_mem,
...
#undef JPU_PAGE_UNIT_SIZE
    ...
    av_dict_set_int(&dict, "sophon_idx", handle->dev_id, 0);
