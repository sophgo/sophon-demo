...
public:
  ...
  void forward_vit(const float *pixel_values, int vit_offset);
...
public:
  ...
  int HIDDEN_SIZE;
  int KV_BYTES = -1; // kv bytes for one token
  int CONV_STATE_BYTES = -1; // conv state bytes for one token
  int NUM_LAYERS;
  int VIT_DIMS;
  int TOKENS_PER_TILE;
  int MAX_PATCHES;
...
private:
  ...
  std::vector<bm_device_mem_t> past_conv_state;
