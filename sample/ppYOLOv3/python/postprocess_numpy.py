...
class PostProcess:
    def __init__(self, conf_thresh=0.1, nms_thresh=0.5, agnostic=False, multi_label=True, max_det=1000):
        ...
        self.nl = 3
        anchors = [[116, 90, 156, 198, 373, 326], [30, 61, 62, 45, 59, 119], [10, 13, 16, 30, 33, 23]] # diff from yolov5
        self.anchor_grid = np.asarray(anchors, dtype=np.float32).reshape(self.nl, 1, -1, 1, 1, 2)
        self.grid = [np.zeros(1)] * self.nl
        self.stride = np.array([32., 16., 8.]) #diff from yolov5
    ...
    def  __call__(self, preds_batch, org_size_batch, m_net_h, m_net_w):
        """
        post-processing
        ...
        for det, (org_w, org_h) in zip(outs, org_size_batch):
            if len(det):
                # Rescale boxes from img_size to im0 size
                coords = det[:, :4]
                coords[:, [0, 2]] *= (float(org_w) / float(m_net_w))
                coords[:, [1, 3]] *= (float(org_h) / float(m_net_h))
