...
int main(int argc, char *argv[]){
  cout.setf(ios::fixed);
  // get params
  const char *keys="{bmodel | ../../models/BM1684/ppyolov3s_v6.1_3output_fp32_1b.bmodel | bmodel file path}"
  ...
  // initialize net
  PPYOLO ppyolov3(bm_ctx);
  CV_Assert(0 == ppyolov3.Init(
        parser.get<float>("conf_thresh"),
        parser.get<float>("nms_thresh"),
        coco_names));

  // profiling
  TimeStamp ppyolov3_ts;
  TimeStamp *ts = &ppyolov3_ts;
  ppyolov3.enableProfile(&ppyolov3_ts);

  // get batch_size
  int batch_size = ppyolov3.batch_size();
  ...
  if (info.st_mode & S_IFDIR){
    ...
    for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++){
      ...
      if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
        // predict
        CV_Assert(0 == ppyolov3.Detect(batch_imgs, boxes));
...
#endif
            // draw image
              ppyolov3.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height, batch_imgs[i]);
      ...
      if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
        ...
        for(int i = 0; i < batch_size; i++){
          ...
          for (auto bbox : boxes[i]) {
            ...
            cout << "  class id=" << bbox.class_id << ", score = " << bbox.score << " (x=" << bbox.x << ",y=" << bbox.y << ",w=" << bbox.width << ",h=" << bbox.height << ")" << endl;
#endif
            ppyolov3.draw_bmcv(h, bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height, batch_imgs[i], false);
          }
  ...
  ppyolov3_ts.calbr_basetime(base_time);
  ppyolov3_ts.build_timeline("ppyolov3 test");
  ppyolov3_ts.show_summary("ppyolov3 test");
  ppyolov3_ts.clear();
