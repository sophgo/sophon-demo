...
PPYOLO::PPYOLO(std::shared_ptr<BMNNContext> context):m_bmContext(context) {
  std::cout << "PPYOLO ctor .." << std::endl;
}

PPYOLO::~PPYOLO() {
  std::cout << "PPYOLO dtor ..." << std::endl;
...
int PPYOLO::Init(float confThresh, float nmsThresh, const std::string& coco_names_file)
{
...
void PPYOLO::enableProfile(TimeStamp *ts)
{
  m_ts = ts;
}

int PPYOLO::batch_size() {
  return max_batch;
};

int PPYOLO::Detect(const std::vector<bm_image>& input_images, std::vector<PPYOLOBoxVec>& boxes)
{
  int ret = 0;
  //3. preprocess
...
int PPYOLO::pre_process(const std::vector<bm_image>& images){
...
float PPYOLO::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool *pIsAligWidth)
{
  float ratio;
...
int PPYOLO::post_process(const std::vector<bm_image> &images, std::vector<PPYOLOBoxVec>& detected_boxes)
{
  PPYOLOBoxVec yolobox_vec;
  ...
  for(int batch_idx = 0; batch_idx < images.size(); ++ batch_idx)
  {
    ...
    for (int i = 0; i < box_num; i++) {
      ...
      {
          float centerX = (ptr[0]+1 - tx1)*frame_width*1.0/m_net_w - 1;
          float centerY = (ptr[1]+1 - ty1)*frame_height*1.0/m_net_h - 1;
          float width = (ptr[2]+0.5) *frame_width*1.0/m_net_w;
          float height = (ptr[3]+0.5) *frame_height*1.0/m_net_h;
    ...
    std::vector<PPYOLOBoxVec> class_vec(m_class_num);
...
int PPYOLO::argmax(float* data, int num){
  float max_value = 0.0;
  int max_index = 0;
  for(int i = 0; i < num; ++i) {
    float value = data[i];
...
float PPYOLO::sigmoid(float x){
  return 1.0 / (1 + expf(-x));
}

void PPYOLO::NMS(PPYOLOBoxVec &dets, float nmsConfidence)
{
  int length = dets.size();
  int index = length - 1;

  std::sort(dets.begin(), dets.end(), [](const PPYOLOBox& a, const PPYOLOBox& b) {
...
void PPYOLO::drawPred(int classId, float conf, int left, int top, int right, int bottom, cv::Mat& frame)   // Draw the predicted bounding box
{
...
void PPYOLO::draw_bmcv(bm_handle_t &handle, int classId, float conf, int left, int top, int width, int height, bm_image& frame, bool put_text_flag)   // Draw the predicted bounding box
{
  if (conf < 0.25) return;
