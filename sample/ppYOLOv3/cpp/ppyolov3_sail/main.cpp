...
int main(int argc, char* argv[]) {
    ...
    // get params
    const char* keys =
        "{bmodel | ../../models/BM1684X/ppyolov3s_v6.1_3output_int8_1b.bmodel | bmodel file path}"
    ...
    // initialize net
    PPYOLO ppyolov3(dev_id, bmodel_file);
    CV_Assert(0 == ppyolov3.Init(parser.get<float>("conf_thresh"), parser.get<float>("nms_thresh"), coco_names));
...
#endif
                    // draw image
                    ppyolov3.draw_bmcv(bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height,
                                     batch_imgs[i], false);
                    // save result
                    json bbox_json;
