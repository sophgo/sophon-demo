...
PPYOLO::PPYOLO(int dev_id, std::string bmodel_file) : engine() {
...
int PPYOLO::Init(float confThresh, float nmsThresh, const std::string& coco_names_file) {
    ...
    if (input_names.size() > 1) {
        std::cout << "InputNumError, ppyolov3 has only one input!" << std::endl;
        exit(1);
    }
...
int PPYOLO::pre_process(sail::BMImage& input) {
    ...
    // Using BMCV api, align with ppyolov3_bmcv.
...
int PPYOLO::pre_process(std::vector<sail::BMImage>& input) {
    if (input.size() != N) {
        std::cout << "Unsupport batch size!" << std::endl;
        exit(1);
    }
...
int PPYOLO::post_process(std::vector<sail::BMImage>& images, std::vector<PPYOLOBoxVec>& detected_boxes) {
...
void PPYOLO::NMS(PPYOLOBoxVec& dets, float nmsConfidence) {
    ...
    std::sort(dets.begin(), dets.end(), [](const PPYOLOBox& a, const PPYOLOBox& b) { return a.score < b.score; });
...
void PPYOLO::drawPred(int classId,
                      float conf,
                      int left,
                      int top,
                      int right,
...
void PPYOLO::draw_bmcv(int classId,
                       float conf,
                       int left,
                       int top,
                       int width,
