...
class ModelExporter:
    def __init__(self, args):
        ...
        self.tts_dir = os.path.join(self.out_dir, f"onnx_tts")
    ...
    def export_tts(self):
        ...
        segs = None
        tts_embedding = np.load("tts_embedding.npy")
        tts_embedding = torch.tensor(tts_embedding)
        for seg in tts.run(tts_embedding.to("cpu"), decoder_topk,
                            None, 
                            codec_chunk_size, codec_padding_size, export_dir=self.tts_dir):
            if segs is None:
                segs = seg
            else:
                segs = torch.cat((segs, seg), dim=2)  
        torchaudio.save("test_stream.wav", segs.squeeze(0).cpu(), sample_rate=24000)
        print("exporting vqvae")
        vqvae_token = torch.zeros(1, 60).to(torch.long)
        vqvae_model = tts.codec_model.vqvae.float()
        torch.onnx.export(vqvae_model,
                    (vqvae_token.unsqueeze(-1), torch.tensor(tts.codec_model.vqvae.h.global_tokens).unsqueeze(0).unsqueeze(0)),
                    self.tts_dir+"/vqvae.onnx",
                    verbose=False,
                    input_names=['x', 'global_style_token'],
                    output_names=['out'],
    ...
    def test_tts(self):
        ...
        for idx, text in enumerate(split_into_sentences(llm_resounse)):
            print("text:", text)
            tts_input_ids = processor.encode(text)
            tts_embedding = self.onnx_rebuilder.embed(torch.tensor(tts_input_ids))
