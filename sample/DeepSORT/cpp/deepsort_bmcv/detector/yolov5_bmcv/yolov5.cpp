...
const std::vector<std::vector<std::vector<int>>> anchors{{{10, 13}, {16, 30}, {33, 23}},
                                                         {{30, 61}, {62, 45}, {59, 119}},
...
int YoloV5::Init(float confThresh, float nmsThresh, std::string coco_names_file) {
...
void YoloV5::drawPred(int trackId,
                      int classId,
                      float conf,
                      int left,
                      int top,
                       ...
                       bool put_text_flag)  // Draw the predicted bounding box
{
    ...
    if (put_text_flag) {
        ...
        //  std::string label =  m_class_names[classId] + ":" + std::to_string(trackId);
        // Display the label at the top of the bounding box
        // int baseLine;
        // cv::Size labelSize = getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseLine);
        // top = std::max(top, labelSize.height);
