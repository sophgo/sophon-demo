...
from yolov5_bmcv import YOLOv5
from deep_sort.deep_sort_bmcv import DeepSort
...
# yolov5 results save in the form of [x1，y1，x2，y2], instead of [x1,y1,w,h]
def update_tracker(target_detector, deepsort, img_batch, frame_id, mot_saver):

    detector_results = target_detector(img_batch)   
    detector_results = [res.astype(np.float32) for res in detector_results]
    imgdraw_batch = []
    for i in range(len(img_batch)):
        detect_num = len(detector_results[i])
        outputs = deepsort.update(detector_results[i], img_batch[i])
...
def argsparser():
    ...
    parser.add_argument('--bmodel_detector', type=str, default='../models/BM1684X/yolov5s_v6.1_3output_int8_4b.bmodel', help='path of detector bmodel')
    parser.add_argument('--bmodel_extractor', type=str, default='../models/BM1684X/extractor_fp16_1b.bmodel', help='path of extractor bmodel')
...
def main():
    ...
    yolov5_args = yolov5_arg(args.bmodel_detector, args.dev_id, cfg.DETECTOR.CONF_THRE, cfg.DETECTOR.NMS_THRE, args.use_cpu_opt)
    ...
    if os.path.isdir(args.input): #test image folder
        ...
        for image_path in image_paths:
            ...
            decoder = sail.Decoder(image_path, True, args.dev_id)
            im = sail.BMImage()
            ret = decoder.read(handle, im)    
            if ret != 0:
                logging.error("{} decode failure.".format(image_path))
                continue
        ...
        while not end_flag:
            ...
            if ret:
                ...
                frame_num += 1
                img_batch.append(im)
            if (frame_num % detector.batch_size == 0 or end_flag) and len(img_batch): 
                frame_id = frame_num + 1 - len(img_batch) # for 4 batch
                result_batch = update_tracker(detector, deepsort, img_batch, frame_id, mot_saver)
                img_batch = []
                start_encode = time.time()
