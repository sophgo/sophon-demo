...
class YOLOworld:
    ...
    def init(self, args):
        ...
        if getattr(args, 'txt_feats_npy', None) and os.path.exists(args.txt_feats_npy):
            # 直接加载预计算文本嵌入 (绕过 clip bmodel; 须与 class_names 顺序一致)
            import numpy as _np
            ce = _np.load(args.txt_feats_npy)
            if ce.ndim == 2: ce = ce[None]
            self.class_embeddings = torch.from_numpy(ce.astype(_np.float32))
            logging.info("load txt_feats from {} shape={}".format(args.txt_feats_npy, tuple(ce.shape)))
        else:
...
def _coco_cat_id(name, cls_idx):
    """类名 -> coco category_id; 非 COCO 类 (open-vocab 自定义) 回退到类索引。"""
    try:
        return COCO_CLASSES.index(name) - 1
    except ValueError:
        return int(cls_idx)
...
def draw_numpy(image, boxes, masks=None, classes_ids=None, conf_scores=None , classes=None):
    for idx in range(len(boxes)):
        ...
        logging.debug("class id={}, score={}, (x1={},y1={},x2={},y2={})".format(_coco_cat_id(classes[classes_ids[idx]], classes_ids[idx]),conf_scores[idx], x1, y1, x2, y2))
    return image
...
def main(args): 
    ...
    if args.class_names == ['all']:
            args.class_names= list(COCO_CLASSES)[1:]  # 80 COCO 类 (去掉 background, 含 toothbrush)
...
def argsparser():
    ...
    parser.add_argument('--bmodel', type=str, default='../models/BM1684X/yoloworld_v2_fp32_1b.bmodel', help='path of bmodel')
    parser.add_argument('--clip_bmodel', type=str, default='../models/BM1684X/clip_text_vitb32_bm1684x_f16_1b.bmodel', help='path of clip')
    parser.add_argument('--txt_feats_npy', type=str, default='', help='precomputed txt_feats npy [1,N,512]; if set, bypass clip bmodel')
