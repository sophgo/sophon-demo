...
class PPOCRv2Rec(object):
    def __init__(self, args):
        ...
        self.net = sail.nn.Engine(model_path, args.dev_id)
        self.stream = sail.nn.Stream(args.dev_id)
        self.graph_name = self.net.get_net_names()[0]
        self.input_name = self.net.get_input_names(self.graph_name)[0]
        self.input_shapes = [self.net.get_input_shapes(self.graph_name, stage)[0] for stage in range(self.net.get_stage_num(self.graph_name))]
        self.input_shape = max(self.input_shapes)
        self.output_name = self.net.get_output_names(self.graph_name)[0]
        self.output_shapes = [self.net.get_output_shapes(self.graph_name, stage)[0] for stage in range(self.net.get_stage_num(self.graph_name))]
        self.output_shape = max(self.output_shapes)
    ...
    def predict(self, tensor):
        ...
        input_data = {0: np.array(tensor, dtype=np.float32)}
        output_data = {0: np.empty(self.output_shape, dtype=np.float32)}
        ret = self.net.process(input_data, output_data, self.stream, self.graph_name)
        assert ret == sail.sailRetCode.SAIL_SUCCESS, "process() fail"
...
def parse_opt():
    ...
    parser.add_argument('--bmodel_rec', type=str, default='../models/BM1690/ch_PP-OCRv3_rec_fp16.bmodel', help='recognizer bmodel path')
