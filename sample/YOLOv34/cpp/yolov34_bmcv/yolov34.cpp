...
Yolo::Yolo(std::shared_ptr<BMNNContext> context):m_bmContext(context) {
...
int Yolo::Detect(const std::vector<bm_image>& input_images, std::vector<YoloBoxVec>& boxes)
{
  int ret = 0;
  //3. preprocess
...
int Yolo::pre_process(const std::vector<bm_image>& images){
...
int Yolo::post_process(const std::vector<bm_image> &images, std::vector<YoloBoxVec>& detected_boxes)
{
...
void Yolo::NMS(YoloBoxVec &dets, float nmsConfidence)
{
  ...
  std::sort(dets.begin(), dets.end(), [](const YoloBox& a, const YoloBox& b) {
...
void Yolo::drawPred(int classId, float conf, int left, int top, int right, int bottom, cv::Mat& frame)   // Draw the predicted bounding box
{
...
void Yolo::draw_bmcv(bm_handle_t &handle, int classId, float conf, int left, int top, int width, int height, bm_image& frame, bool put_text_flag)   // Draw the predicted bounding box
{
  if (conf < 0.25) return;
  ...
  else bmcv_image_draw_rectangle(handle, frame, 1, &rect, thickness, colors[classId % colors_num][0], colors[classId % colors_num][1], colors[classId % colors_num][2]);
  if (put_text_flag){
    //Get the label for the class name and its confidence
