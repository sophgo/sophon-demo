...
Yolo::Yolo(int dev_id, std::string bmodel_file) : engine() {
    ...
    std::cout << "Yolo ctor .." << std::endl;
}

Yolo::~Yolo() {
    std::cout << "Yolo dtor ..." << std::endl;
}

int Yolo::Init(float confThresh, float nmsThresh, const std::string& coco_names_file, int anchors_type) {
    m_confThreshold = confThresh;
    m_nmsThreshold = nmsThresh;
    m_anchors_type = anchors_type;
    ...
    if (input_names.size() > 1) {
        std::cout << "InputNumError, yolov34 has only one input!" << std::endl;
        exit(1);
    }
...
int Yolo::Detect(std::vector<sail::BMImage>& input_images, std::vector<YoloBoxVec>& boxes) {
    int ret = 0;
    // 1. preprocess
    m_ts->save("yolov34 preprocess", input_images.size());
    ...
    // 2. forward
    m_ts->save("yolov34 inference", input_images.size());
    engine->process(graph_names[0], input_tensors, output_tensors);
    m_ts->save("yolov34 inference", input_images.size());

    // 3. post process
    m_ts->save("yolov34 postprocess", input_images.size());
...
int Yolo::pre_process(sail::BMImage& input) {
    int ret = 0;
    ...
    sail::BMImage resized_img(engine->get_handle(), input_shape[2], input_shape[3], FORMAT_RGB_PLANAR, DATA_TYPE_EXT_1N_BYTE);
    resized_img.align();
#if USE_ASPECT_RATIO
...
int Yolo::pre_process(std::vector<sail::BMImage>& input) {
    int ret = 0;
    if (input.size() != N) {
        std::cout << "Unsupport batch size!" << std::endl;
        exit(1);
    ...
    for (size_t i = 0; i < input.size(); ++i) {
        sail::BMImage rgb_img(engine->get_handle(), input[i].height(), input[i].width(), FORMAT_RGB_PLANAR, DATA_TYPE_EXT_1N_BYTE);
        rgb_img.align();
...
float Yolo::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int Yolo::post_process(std::vector<sail::BMImage>& images, std::vector<YoloBoxVec>& detected_boxes) {
    YoloBoxVec yolobox_vec;
    ...
    for (int batch_idx = 0; batch_idx < images.size(); ++batch_idx) {
        ...
        if (min_dim == 5) {
            ...
            std::vector<std::vector<std::vector<int>>> anchors{{{10, 13}, {16, 30}, {33, 23}},{{30, 61}, {62, 45}, {59, 119}},{{116, 90}, {156, 198}, {373, 326}}};
            if(m_anchors_type ==1){
        ...
        std::vector<YoloBoxVec> class_vec(m_class_num);
...
int Yolo::argmax(float* data, int num) {
    float max_value = 0.0;
    int max_index = 0;
    for (int i = 0; i < num; ++i) {
        float value = data[i];
...
void Yolo::NMS(YoloBoxVec& dets, float nmsConfidence) {
    int length = dets.size();
    int index = length - 1;

    std::sort(dets.begin(), dets.end(), [](const YoloBox& a, const YoloBox& b) { return a.score < b.score; });
...
void Yolo::drawPred(int classId,
                      float conf,
                      int left,
                      int top,
                      int right,
...
void Yolo::draw_bmcv(int classId,
                       float conf,
                       int left,
                       int top,
                       int width,
