...
int main(int argc, char* argv[]) {
    ...
    // get params
    const char* keys =
        "{bmodel | ../../models/BM1684X/yolov3_int8_1b.bmodel | bmodel file path}"
    ...
    // initialize net
    Yolo yolov34(dev_id, bmodel_file);
    CV_Assert(0 == yolov34.Init(parser.get<float>("conf_thresh"), parser.get<float>("nms_thresh"), coco_names, anchors_type));
...
#endif
                    // draw image
                    yolov34.draw_bmcv(bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height,
                                     batch_imgs[i], false);
                    // save result
                    json bbox_json;
