...
```
测试结束后，会将预测的图片保存在`results/images`下，预测的结果保存在`results/yolov3_fp32_1b.bmodel_test_opencv_python_result.json`下，同时会打印预测结果、推理时间等信息。
...
注意，riscv平台暂不支持用opencv进行视频测试，但是您可以选择`yolov34_bmcv.py`测试。
