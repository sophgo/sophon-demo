...
class PreprocessYOLO(object):
    ...
    def _load_and_resize(self, image):
        ...
        image_raw = Image.fromarray(np.uint8(image))
    ...
    '''
    preprocessing with OpenCV, if used, the later processing in model.py should be modified
    '''
    def _resize_shuffle_and_normalize(self, image):

        image_raw = image.copy()
        # Resize long
        img = cv2.resize(image, (self.yolo_input_resolution[1], self.yolo_input_resolution[0]), interpolation=cv2.INTER_LINEAR)
        # BGR => RGB
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # to numpy
        img_rgb_data = img_rgb.astype(np.float32)
        # Normalize to [0,1]
        img_rgb_data /= 255.0
        # HWC to CHW format:
        img_rgb_data = np.transpose(img_rgb_data, [2, 0, 1])
        # CHW to NCHW format
        img_rgb_data = np.expand_dims(img_rgb_data, axis=0)
        # Make sure the image memory is contiguous:
        image_preprocessed = np.ascontiguousarray(img_rgb_data)
        return image_raw, image_preprocessed
...
class PostprocessYOLO(object):
    ...
    def process(self, outputs, resolution_raw):
        """Take the YOLOv3 outputs generated from a engine forward pass, post-process them
        ...
        Keyword arguments:
        outputs -- outputs from a engine in NCHW format
    ...
    def _reshape_output(self, output):
        """Reshape an output from NCHW to NHWC format (with expected C=255),
        and then return it in (height,width,3,85) dimensionality after further reshaping.

        Keyword argument:
        output -- an output from an engine after inference
        """
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        box_xy = output_reshaped[..., :2]
        box_wh = exponential_v(output_reshaped[..., 2:4]) * anchors_tensor
        box_class_probs = (output_reshaped[..., 5:])
        box_confidence = (output_reshaped[..., 4])
