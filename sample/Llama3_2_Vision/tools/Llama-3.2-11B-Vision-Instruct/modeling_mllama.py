...
def _prepare_aspect_ratio_attention_mask(
    ...
    # avoid scatterND to generate dynamic subnet
    batch_size, max_num_tiles = 1, 4
    attention_mask = aspect_ratio_mask.view(batch_size, max_num_tiles, 1, 1).to(dtype)
    attention_mask = attention_mask.repeat(1, 1, num_patches, 1)
    pad_patches = target_length - num_patches
    attention_mask = torch.cat(
        [attention_mask, torch.zeros(1, max_num_tiles, pad_patches, 1)], axis=2)
    ...
    attention_mask = attention_mask @ attention_mask.transpose(-1, -2) * (-10000.)
...
class MllamaSelfAttentionDecoderLayer(nn.Module):
    def __init__(self, config: MllamaTextConfig, layer_idx: int):
        ...
        self.self_attn = MLLAMA_TEXT_ATTENTION_CLASSES["eager"](config=config, layer_idx=layer_idx)
...
class MllamaCrossAttentionDecoderLayer(torch.nn.Module):
    ...
    def __init__(self, config: MllamaTextConfig, layer_idx: int) -> None:
        super().__init__()
        self.layer_idx = layer_idx
        self.cross_attn = MLLAMA_TEXT_CROSS_ATTENTION_CLASSES["eager"](config, layer_idx=layer_idx)
    ...
    def forward(
        ...
        if full_text_row_masked_out_mask is not None:
            hidden_states = full_text_row_masked_out_mask * hidden_states  # type: ignore
...
class MllamaVisionModel(MllamaPreTrainedModel):
    ...
    def apply_class_embedding(self, hidden_state: torch.Tensor) -> torch.Tensor:
        batch_size, num_tiles, _, hidden_size = 1, 4, 1600, 1280
        class_embedding = self.class_embedding.expand(batch_size, num_tiles, 1, hidden_size)
        hidden_state = torch.cat([class_embedding, hidden_state], dim=2)
        return hidden_state
    ...
    def forward(
        ...
        batch_size, num_concurrent_media = 1, 1
        num_tiles, num_channels, height, width = 4,3,560,560 #pixel_values.shape
        ...
        # Tile embeddings
        num_patches, dim = 1600, 1280#hidden_state.shape
        ...
        num_padding_patches = 7 #(8 - (hidden_state.shape[-2] % 8)) % 8
        slice_index = -num_padding_patches
        # Compute padding tuple for pad function
        # padding = (0, 0, 0, num_padding_patches)  # (pad_left, pad_right, pad_left for dim -2, pad_right for dim -2)
        # Pad the tensor
        # hidden_state = F.pad(hidden_state, padding, mode="constant", value=0)
        hidden_state = torch.cat([hidden_state, torch.zeros(1, num_tiles, num_padding_patches, dim)], axis=2)
        ...
        attention_mask = _prepare_aspect_ratio_attention_mask(
            ...
            target_length=num_patches+num_padding_patches,
            dtype=self.dtype,
        )

        # Apply encoder
        hidden_state = hidden_state.view(batch_size * num_concurrent_media, num_tiles * (num_patches + num_padding_patches), dim)
        ...
        intermediate_hidden_states = intermediate_hidden_states.reshape(
            batch_size * num_concurrent_media, num_tiles, num_patches + num_padding_patches, len(self.intermediate_layers_indices) * dim
        )
        intermediate_hidden_states = intermediate_hidden_states[:, :, :slice_index]
        intermediate_hidden_states = intermediate_hidden_states.reshape(
            batch_size * num_concurrent_media, num_tiles, num_patches, len(self.intermediate_layers_indices) * dim
        )
