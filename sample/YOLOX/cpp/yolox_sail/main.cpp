...
int main(int argc, char* argv[]) {
    ...
    // get params
    const char* keys =
        "{bmodel | ../../models/BM1684X/yolox_s_fp16_1b.bmodel | bmodel file path}"
    ...
    // initialize net
    YoloX yolox(dev_id, bmodel_file);
    CV_Assert(0 == yolox.Init(parser.get<float>("conf_thresh"), parser.get<float>("nms_thresh"), coco_names));

    // profiling
    TimeStamp yolox_ts;
    TimeStamp* ts = &yolox_ts;
    yolox.enableProfile(&yolox_ts);

    // get batch_size
    int batch_size = yolox.batch_size();
    ...
    if (info.st_mode & S_IFDIR) {
        ...
        for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++) {
            ...
            iter--;
            CV_Assert(0 == yolox.Detect(batch_imgs, boxes));
...
#endif
                    // draw image
                    yolox.draw_bmcv(bbox.class_id, bbox.score, bbox.x, bbox.y, bbox.width, bbox.height,
                                     batch_imgs[i], false);
                    // save result
                    json bbox_json;
    ...
    yolox_ts.build_timeline("yolox test");
    yolox_ts.show_summary("yolox test");
    yolox_ts.clear();
