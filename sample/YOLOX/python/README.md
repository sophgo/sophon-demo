...
| 序号 |  Python例程      | 说明                                |
| ---- | --------------- | -----------------------------------  |
| 1    | yolox_opencv.py | 使用OpenCV解码、OpenCV前处理、SAIL推理 |
| 2    | yolox_bmcv.py   | 使用SAIL解码、BMCV前处理、SAIL推理 |
...
### 2.1 参数说明
yolox_opencv.py和yolox_bmcv.py的参数一致，以yolox_opencv.py为例：
```bash
...
```
测试结束后，会将预测的图片保存在`results/images`下，预测的结果保存在`results/yolox_s_fp32_1b.bmodel_test_opencv_python_result.json`下，同时会打印预测结果、推理时间等信息。
...
`yolox_bmcv.py`会将预测结果画在图片上并保存在`results/images`中。
