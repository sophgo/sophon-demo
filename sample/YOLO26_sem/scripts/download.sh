...
if [ ! -d "../datasets" ]; then
    ...
    python3 -m dfss --url=open@sophgo.com:sophon-demo/common/cityscapes.tar.gz
    tar xvf cityscapes.tar.gz && rm cityscapes.tar.gz
    python3 -m dfss --url=open@sophgo.com:sophon-demo/YOLO26_sem/cali.tar.gz
    tar xvf cali.tar.gz && rm cali.tar.gz
    python3 -m dfss --url=open@sophgo.com:sophon-demo/common/cityscapes_video.avi  # test video
    popd
...
if [ ! -d "../models/BM1684X" ]; then
    if [ $download_bm1684x == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/YOLO26_sem/BM1684X.tar.gz
...
if [ ! -d "../models/BM1688" ]; then
    if [ $download_bm1688 == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/YOLO26_sem/BM1688.tar.gz
...
if [ ! -d "../models/CV186X" ]; then
    if [ $download_cv186x == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/YOLO26_sem/CV186X.tar.gz
...
if [ ! -d "../models/onnx" ]; then
    if [ $download_onnx == 1 ]; then
        python3 -m dfss --url=open@sophgo.com:sophon-demo/YOLO26_sem/onnx.tar.gz
