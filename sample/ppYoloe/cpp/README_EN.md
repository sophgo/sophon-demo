...
# Example of PPYOLOE with Sophon Inference
...
| ---- | ------------- | -----------------------------------  |
| 1    | ppyoloe_bmcv | Decoding with FFmpeg, preprocessing with BMCV, inference with BMRT   |
| 2    | ppyoloe_sail | Decoding with SAIL, preprocessing with SAIL, inference with SAIL   |
...
cd ..
```
After the compilation is complete, ppyoloe_bmcv.pcie will be generated in the ppyoloe_bmcv directory.
...
cd ..
```
After the compilation is complete, ppyoloe_sail.pcie will be generated in the ppyoloe_bmcv directory.
### 2.2 SoC Platform
...
make
```
After the compilation is complete, ppyoloe_bmcv.soc will be generated in the ppyoloe_bmcv directory.
#### 2.2.2 sail
...
make
```
After the compilation is complete, ppyoloe_sail.soc will be generated in the ppyoloe_bmcv directory.
...
The executable program has a set of parameters by default. Please pay attention to pass the parameters according to the actual situation. `ppyoloe_bmcv.pcie and ppyoloe_sail.pcie has same parameters.` The specific parameters are described as follows:
```bash
...
```
> **Note:** The method of passing parameters in C++ is different from that in Python. The method of passing parameters in C++ needs to use the equal sign. For example `./ppyoloe_bmcv.pcie --bmodel=xxx`.
...
```bash
./ppyoloe_bmcv.pcie --input=../../datasets/test --bmodel=../../models/BM1684/ppyoloe_fp32_1b.bmodel --dev_id=0 --conf_thresh=0.5 --nms_thresh=0.5 --obj_thresh=0.5 --classnames=../../datasets/coco.names 
```
After the test, the predicted images will be saved in `results/images`, the predicted results will be saved in `results/ppyoloe_fp32_1b.bmodel_test_bmcv_cpp_result.json`, and the prediction results, inference time and other information will be printed at the same time.
...
```bash
./ppyoloe_bmcv.pcie --input=../../datasets/test_car_person_1080P.mp4 --bmodel=../../models/BM1684/ppyoloe_fp32_1b.bmodel --dev_id=0 --conf_thresh=0.5 --nms_thresh=0.5 --obj_thresh=0.5 --classnames=../../datasets/coco.names
```
