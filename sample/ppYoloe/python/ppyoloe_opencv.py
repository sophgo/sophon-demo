...
class PPyoloe:
    ...
    def preprocess(self, img):
        ...
        return chw_img.astype(np.float32), ratio

    def predict(self, input_img, img_num, ratio_list):
        input_data = {self.input_name: input_img, "scale_factor": np.array(ratio_list)}
    ...
    def __call__(self, img_list):
        ...
        start_time = time.time()
        outputs = self.predict(input_img, img_num, ratio_list)
...
def main(args):
    ...
    # warm up
    # for i in range(10):
    #     results = ppyoloe([np.zeros((640, 640, 3))])
    ppyoloe.init()
