...
class PostProcess:
    def __init__(self, conf_thresh=0.5, nms_thresh=0.5, agnostic=False, multi_label=True, max_det=1000):
    ...
    def  __call__(self, preds_batch, org_size_batch, ratios_batch):
        """
        post-processing
        ...
        :return:
        """
        if isinstance(preds_batch, list) and len(preds_batch) != 2:
            print('preds_batch type: '.format(type(preds_batch)))
            raise NotImplementedError
        data1 = preds_batch[0]
        data2 = preds_batch[1]
        data1_transposed = np.transpose(data1, (0, 2, 1))

        max_values_per_row = np.expand_dims(np.amax(data1_transposed, axis=2), axis=2)
        dets = np.concatenate((data2, max_values_per_row, data1_transposed), axis=2)
        data = dets[0]
        ...
        for det, (org_w, org_h), in zip(outs, org_size_batch):
            if len(det):
                # Rescale boxes from img_size to im0 size
                coords = det[:, :4]
