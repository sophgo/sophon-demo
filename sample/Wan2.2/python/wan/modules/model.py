...
# DEVICE: Modify complex number calculation method to adapt to TPU
@torch.amp.autocast('tpu', enabled=False)
def rope_apply(x, grid_sizes, freqs):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        seq_len = f * h * w
        xi = x[i, :seq_len]
        xi = xi.view(seq_len, N, C, 2)

        c0h = c0 // 2
        c1h = c1 // 2
        c2h = c2 // 2
        cos = torch.cat([
            cos0[:f].view(f, 1, 1, c0h).expand(f, h, w, c0h),
            cos1[:h].view(1, h, 1, c1h).expand(f, h, w, c1h),
            cos2[:w].view(1, 1, w, c2h).expand(f, h, w, c2h),
        ], dim=-1).reshape(seq_len, -1).to(dtype=dtype, device=device)
        sin = torch.cat([
            sin0[:f].view(f, 1, 1, c0h).expand(f, h, w, c0h),
            sin1[:h].view(1, h, 1, c1h).expand(f, h, w, c1h),
            sin2[:w].view(1, 1, w, c2h).expand(f, h, w, c2h),
        ], dim=-1).reshape(seq_len, -1).to(dtype=dtype, device=device)

        cos_full = torch.repeat_interleave(cos, repeats=1, dim=1).unsqueeze(1)
        sin_full = torch.repeat_interleave(sin, repeats=1, dim=1).unsqueeze(1)

        x_even = xi[..., 0]
        x_odd  = xi[..., 1]
        x_even_rot = x_even * cos_full - x_odd * sin_full
        x_odd_rot  = x_even * sin_full + x_odd * cos_full
        x_rot = torch.stack([x_even_rot, x_odd_rot], dim=-1).reshape(seq_len, N, 2 * C)

        x_out = torch.cat([x_rot, x[i, seq_len:]], dim=0)
...
#         output.append(x_i)
#     return torch.stack(output).float().to(dev)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        # DEVICE: Modify the freqs generation method
        self.freqs = build_freqs_real(1024, dim // num_heads, dtype=torch.float32)
    ...
    def forward(
        ...
        # embeddings
        x = [u.to(torch.bfloat16) for u in x]
