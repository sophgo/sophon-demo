...
from utils import draw_numpy, single_encode
import logging
...
class Yolo26Seg:
    def __init__(self, args):
        ...
        self.postprocess = PostProcess(conf_thres=self.conf_thresh)
...
def main(args):
    ...
    if os.path.isdir(args.input): 
        ...
        results_list = []
        cn = 0
        yolo26_handle = sail.Handle(args.dev_id)
        for root, dirs, filenames in os.walk(args.input):
            for filename in filenames:
                ...
                bmimg = sail.BMImage()
                ret = decoder.read(yolo26_handle, bmimg)
                ...
                if (len(bmimg_list) == batch_size or cn == len(filenames)) and len(bmimg_list):
                    ...
                    for i, filename in enumerate(filename_list):
                        ...
                        # yolo26.postprocess.draw_and_visualize(save_name,bmimg_list[i].asmat(), boxes, segments, vis=False, save=True)
                        # save image
                        img = bmimg_list[i].asmat()
                        if boxes.shape[0] >= 1:
                            res_img = draw_numpy(img, boxes[:,:4], masks=masks, classes_ids=boxes[:, -1], conf_scores=boxes[:, -2])
                        else:
                            res_img = img
    ...
    # test videos
        ...
        while not end_flag:
            frame = sail.BMImage()
            start_time = time.time()
            ret = decoder.read(yolo26_handle, frame)
            decode_time += time.time() - start_time
            ...
            if (len(frame_list) == batch_size or end_flag) and len(frame_list):
                ...
                for i, frame in enumerate(frame_list):
                    ...
                    if boxes.shape[0] >= 1:
                        drawed_mat = draw_numpy(img, boxes[:,:4], masks=masks, classes_ids=boxes[:, -1], conf_scores=boxes[:, -2])
                    else:
                        drawed_mat = img
                    drawed_bmimg = yolo26.bmcv.mat_to_bm_image(drawed_mat)
