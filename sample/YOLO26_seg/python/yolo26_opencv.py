            ...
            for filename in filenames:
                ...
                if (len(img_list) == batch_size or cn == len(filenames)) and len(img_list):
                    ...
                    for i, filename in enumerate(filename_list):
                        det_draw, masks = results[i]

                        # if det_draw is None:
                        #     res_dict ={'image_name': filename, 'bboxes': [], 'segs': []}
                        #     results_list.append(res_dict)
                        #     continue
                        
                        # save image
                        if det_draw.shape[0] >= 1:
                            res_img = draw_numpy(img_list[i], det_draw[:,:4], masks=masks, classes_ids=det_draw[:, -1], conf_scores=det_draw[:, -2])
                        else:
                            res_img = img_list[i]
        ...
        while not end_flag:
            ...
            if (len(frame_list) == batch_size or end_flag) and len(frame_list):
                ...
                for i, frame in enumerate(frame_list):
                    ...
                    # save image
                    if det_draw.shape[0] >= 1:
                        res_frame = draw_numpy(frame_list[i], det_draw[:,:4], masks=masks, classes_ids=det_draw[:, -1], conf_scores=det_draw[:, -2])
                    else:
                        res_frame = frame_list[i]
                    out.write(res_frame)
                frame_list.clear()
        cap.release()
        out.release()
