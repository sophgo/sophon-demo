...
private:
    ...
    int post_process(const std::vector<bm_image>& input_images, 
                     ...
                     std::vector<Yolo26BoxVec>& boxes);
    static float get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* alignWidth);
    void xywh2xyxy(Yolo26BoxVec& xyxyboxes, std::vector<std::vector<float>> box);
    static bool Yolo26Box_cmp(Yolo26Box a, Yolo26Box b);
    void clip_boxes(Yolo26BoxVec& yolobox_vec, int src_w, int src_h);
...
public:
    int batch_size = -1;
    TimeStamp* m_ts = NULL;
    Yolo26_seg(std::string bmodel_file, std::string coco_names_file, int dev_id = 0, float confThresh = 0.25){
        ...
        // m_class_num rely on m_mask_num.
        for (int i = 0; i < netinfo->output_num; i++) {
            auto& shape = netinfo->stages[0].output_shapes[i];
            if (shape.num_dims == 4) {
                m_mask_dim = shape.dims[1];
            }
        }
        for (int i = 0; i < netinfo->output_num; i++) {
            auto& shape = netinfo->stages[0].output_shapes[i];
            if (shape.num_dims == 3) {
                m_class_num = shape.dims[2] - 4 - m_mask_dim;
            }
        }
        if (m_mask_dim == -1 || m_class_num == -1) {
    ...
    void draw_result(cv::Mat& img, Yolo26BoxVec& result);
};
