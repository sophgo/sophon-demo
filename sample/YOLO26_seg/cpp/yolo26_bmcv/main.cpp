...
int main(int argc, char* argv[]) {
    ...
    // initialize net
    Yolo26_seg yolo26(bmodel_file, coco_names, dev_id, parser.get<float>("conf_thresh"));
    ...
    if (info.st_mode & S_IFDIR) {
        ...
        for (vector<string>::iterator iter = files_vector.begin(); iter != files_vector.end(); iter++) {
            ...
            if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
                // predict
                auto ret = yolo26.Detect(batch_imgs, boxes);
                assert(0 == ret);

                for (int i = 0; i < batch_imgs.size(); i++) {
                    yolo26.draw_result(batch_mats[i], boxes[i]);
