...
int Yolo26_seg::Detect(const std::vector<bm_image>& input_images, std::vector<Yolo26BoxVec>& boxes) {
    ...
    m_ts->save("yolo26 preprocess", input_images.size());
    ret = pre_process(input_images, input_tensor, txy_batch, ratios_batch);
    assert(ret == 0);
    m_ts->save("yolo26 preprocess", input_images.size());

    m_ts->save("yolo26 inference", input_images.size());
    ret = forward(input_tensor, output_tensors);
    assert(ret == 0);
    m_ts->save("yolo26 inference", input_images.size());

    m_ts->save("yolo26 postprocess", input_images.size());
...
float Yolo26_seg::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int Yolo26_seg::pre_process(const std::vector<bm_image>& images, 
                            bm_tensor_t& input_tensor, 
                            std::vector<std::pair<int, int>>& txy_batch, 
                            std::vector<std::pair<float, float>>& ratios_batch) {
    int ret = 0;
...
int Yolo26_seg::forward(bm_tensor_t& input_tensor, std::vector<bm_tensor_t>& output_tensors){
    // static int count = 0;
 ...
 */
float* Yolo26_seg::get_cpu_data(bm_tensor_t* tensor, float scale){
    int ret = 0;
    float *pFP32 = NULL;
...
int Yolo26_seg::post_process(const std::vector<bm_image>& input_images, 
                             std::vector<bm_tensor_t>& output_tensors, 
                             const std::vector<std::pair<int, int>>& txy_batch, 
                             const std::vector<std::pair<float,float>>& ratios_batch,
                             std::vector<Yolo26BoxVec>& detected_boxes) {
    float* data_box = NULL;
    float* data_mask = NULL;
    bm_tensor_t tensor_box;
    bm_tensor_t tensor_mask;
    for(int i = 0; i < output_tensors.size(); i++) {
        if(output_tensors[i].shape.num_dims == 4) {
            tensor_mask = output_tensors[i];
            data_mask = get_cpu_data(&output_tensors[i], netinfo->output_scales[i]);
        }else if(output_tensors[i].shape.num_dims == 3){
    ...
    for (int batch_idx = 0; batch_idx < input_images.size(); ++batch_idx) {
        ...
        float* batch_data_box =  data_box + batch_idx * box_num * nout; //output_tensor: [bs, box_num, 6]
        ...
        for (int i = 0; i < box_num; i++) {
            ...
            box.mask.insert(box.mask.end(), 
                            batch_data_box + box_index + (nout - m_mask_dim), 
                            batch_data_box + box_index + nout);
        ...
        cv::Mat batch_mask(tensor_mask.shape.num_dims, 
                        tensor_mask.shape.dims, 
                        CV_32F, 
                        data_mask + batch_idx * tensor_mask.shape.dims[1] * tensor_mask.shape.dims[2] * tensor_mask.shape.dims[3]); 
        
        ImageInfo para = {cv::Size(frame_width, frame_height), {ratio_x, ratio_y, tx1, ty1}};
        Yolo26BoxVec yolobox_vec_tmp;
        for (int i = 0; i < yolobox_vec.size(); i++) {
            if (yolobox_vec[i].x2 > yolobox_vec[i].x1 + 1 && yolobox_vec[i].y2 > yolobox_vec[i].y1 + 1) {
                get_mask(cv::Mat(yolobox_vec[i].mask).t(), 
                         batch_mask, 
                         para,
...
void Yolo26_seg::get_mask(const cv::Mat& mask_info,
...
void Yolo26_seg::clip_boxes(Yolo26BoxVec& yolobox_vec, int src_w, int src_h) {
    for (int i = 0; i < yolobox_vec.size(); i++) {
...
void Yolo26_seg::xywh2xyxy(Yolo26BoxVec& xyxyboxes, std::vector<std::vector<float>> box) {
...
void Yolo26_seg::draw_result(cv::Mat& img, Yolo26BoxVec& result) {
