...
usage() 
{
  echo "Usage: $0 [ -m MODE compile_nntc|compile_mlir|pcie_test|soc_build|soc_test] [ -t TARGET BM1684|BM1684X|CV84X6|BM1688|CV186X] [ -s SOCSDK] [-a SAIL] [ -d TPUID] [ -p PYTEST auto_test|pytest]" 1>&2 
}
...
function bmrt_test_benchmark(){
    ...
    if test $TARGET = "BM1684"; then
      ...
      bmrt_test_case CV84X6/segformer.b0.512x1024.city.160k_fp16_1b.bmodel
      bmrt_test_case CV84X6/segformer.b0.512x1024.city.160k_int8_1b.bmodel
...
then
  ...
  then
    # CV84X6当前固件不支持FP32，仅FP16/INT8。segformer_sail.soc基于sophon-sail C++库交叉编译
