...
float YoloV8_cls::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int YoloV8_cls::pre_process(const std::vector<bm_image>& images, 
...
int YoloV8_cls::forward(bm_tensor_t& input_tensor, bm_tensor_t& output_tensor){
    // static int count = 0;
 ...
 */
float* YoloV8_cls::get_cpu_data(bm_tensor_t* tensor, float scale){
    int ret = 0;
    float *pFP32 = NULL;
...
int YoloV8_cls::post_process(const std::vector<bm_image>& input_images, 
