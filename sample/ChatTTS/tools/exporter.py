...
def export_gpt():
    ...
    gpt_model.from_pretrained(os.path.join(args.source_dir, asdict(chattts_config.path)["gpt_ckpt_path"]))
...
def export_decoder():
    ...
    decoder.load_state_dict(
        torch.load(
            os.path.join(args.source_dir, asdict(chattts_config.path)["decoder_ckpt_path"]),
            weights_only=True,
            mmap=True,
        )
    )
...
def export_vocos():
    ...
    vocos.load_state_dict(
        torch.load(
            os.path.join(args.source_dir, asdict(chattts_config.path)["vocos_ckpt_path"]), weights_only=True, mmap=True
        )
    )
