...
class BeamSearchSeq2SeqGenerator(Seq2SeqGenerator):
    ...
    def __call__(
        ...
        logging.info('offline encode cost time(ms): ' + str((time.time() - start_time) * 1000))
        ...
        hypotheses = op()
        logging.info('offline all decoder steps cost time(ms): ' + str((time.time() - start_time) * 1000))
...
class _BeamSearchSequenceGeneratorOpBase(ABC):
    ...
    def _reorder_state(self, new_order: Tensor, handle: sail.Handle=None) -> None:
        self.state_bag.reorder(new_order, handle)
...
class _BeamSearchSeq2SeqGeneratorOp(_BeamSearchSequenceGeneratorOpBase):
    ...
    @override
    def _reorder_state(self, new_order: Tensor, handle: sail.Handle=None) -> None:
        super()._reorder_state(new_order, self.handle)

        # self.encoder_output = self.encoder_output.index_select(dim=0, index=new_order)
        # tmp_tensor = sail.Tensor(self.handle, self.encoder_output.shape(), self.encoder_output.dtype, False, True)
        # tmp_tensor.sync_d2d(self.encoder_output, 0, 0, self.encoder_output.shape()[0] * self.encoder_output.shape()[1] * self.encoder_output.shape()[2])
        # for indx, i in enumerate(new_order):
        #     self.encoder_output.sync_d2d(tmp_tensor, i.item() * self.encoder_output.shape()[1] * self.encoder_output.shape()[2], indx * self.encoder_output.shape()[1] * self.encoder_output.shape()[2], self.encoder_output.shape()[1] * self.encoder_output.shape()[2])
        ...
        if self.encoder_padding_mask is not None:
            ...
            self.encoder_padding_mask = PaddingMask(
                encoder_seq_lens, batch_seq_len=self.encoder_output.shape()[1]
            )
