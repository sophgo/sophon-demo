...
def load_audio(file: str, sr: int = SAMPLE_RATE) -> np.ndarray:
    ...
    """
    try:
        # Load audio file. Prefer soundfile: torchaudio>=2.11 defaults to a
        # torchcodec backend that is often absent on aarch64 SoC images and
        # ignores the backend= kwarg, so torchaudio.load() raises ImportError.
        try:
            import soundfile as sf
            data, sample_rate = sf.read(file, dtype="float32")  # (samples,) or (samples, channels)
            if data.ndim > 1:
