...
class Whisper():
    def __init__(self, dims: ModelDimensions, args):
        ...
        # prefer chip-native bmodel (1688 on BM1688 SoC), fall back to 1684x
        combined_whisper_model_path = None
        for _arch in ("1688", "1684x"):
            _p = os.path.join(self.bmodel_dir, f"bmwhisper_{self.model_name}_{_arch}_f16.bmodel")
            if os.path.exists(_p):
                combined_whisper_model_path = _p
                break
        assert combined_whisper_model_path is not None, (
            f"neither bmwhisper_{self.model_name}_1688_f16.bmodel nor "
            f"bmwhisper_{self.model_name}_1684x_f16.bmodel found in {self.bmodel_dir}"
        )
