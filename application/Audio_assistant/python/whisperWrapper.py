...
from bmwhisper.timing import add_word_timestamps
...
class WhisperWrapper():
    ...
    def load_model(self):
        ...
        pop_list = ["model_name", "model_dir", "bmodel_dir", "chip_mode", "chip", "profile"]
        for arg in pop_list:
            args.pop(arg)
    ...
    def transcribe(self, audio_path):
        ...
        # self.writer(result, audio_path, self.writer_args)
        cpu_time = time.time() - audio_start_time - self.model.time
        if self.loop_profile:
            self.model.print_cnt()
        # if self.args["profile"]:
        #     print()
        #     print(f"Total tpu inference time: {self.model.time}s")
        #     print(f"Total cpu inference time: {cpu_time}s")
        #     print(f"Total time: {cpu_time + self.model.time}s")
        self.model.time = 0
...
def set_argparser():
    ...
    parser.add_argument("--padding_size", type=optional_int, default=128, help="max pre-allocation size for the key-value cache")
