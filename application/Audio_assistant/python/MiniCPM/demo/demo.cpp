...
class MiniCPM {
public:
  void init(std::string model_path, std::string tokenizer_path, int devid);
  void chat();
  void deinit();
...
void MiniCPM::init(std::string model_path, std::string tokenizer_path, int devid) {
...
void MiniCPM::answer(const std::string &input_str) {
  ...
  while (token != EOS && token_length < SEQLEN) {
    ...
    // if (diff == "。" || diff == "，" || diff == "." || diff == "," || diff == "?" || diff == "？" || diff == "!" || diff == "！" || diff == ":" || diff == "：")
    // best for listen
    if (diff == "。" || diff == "." || diff == "?" || diff == "？" || diff == "!" || diff == "！" || diff == ":" || diff == "：")
...
void processArguments(int argc, char *argv[], std::string &model_path,
                      std::string &tokenizer_path, int &devid) {
...
int main(int argc, char **argv) {
  ...
  int devid;
  processArguments(argc, argv, model_path, tokenizer_path, devid);
  if (model_path.empty()) {
    Usage();
    exit(EXIT_FAILURE);
  }
  ...
  minicpm.init(model_path, tokenizer_path, devid);
  printf("==========================\n");
  minicpm.chat();
  minicpm.deinit();
  return 0;
