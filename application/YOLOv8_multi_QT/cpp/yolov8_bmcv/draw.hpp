...
void draw_json(std::shared_ptr<DataPost> box_data, std::shared_ptr<cv::Mat> image, std::vector<std::string>& class_names){
  ...
  for (auto& bbox: boxes){
    drawPred(bbox.class_id, bbox.score, bbox.x1, bbox.y1, bbox.x2, bbox.y2, *image, class_names);
  }
...
std::mutex mtx_draw;
// 使用yolov8后处理后的数据进行操作
void worker_draw(YOLOv8& yolov8, std::vector<std::string>& class_names, VideoConsole<cv::Mat>* video_console = nullptr){
  while (true){
    auto box_data = std::make_shared<DataPost>();
    auto origin_image = std::make_shared<cv::Mat>();

    int ret = yolov8.get_post_data(box_data, origin_image);
    if (ret){
      return;
    }
