...
void worker_sink(YOLOv8& yolov8){
  while (true){
...
#if DRAW_ACC
  auto thread_draw = std::thread([&]{worker_draw(yolov8, class_names, &video_console);});
  //不建议使用 join 函数：调用 join() 会使调用线程（通常是主线程）阻塞，直到被调用的线程完成执行。
  //但被调用的线程 thread_draw 内部是死循环，会导致主线程一直被阻塞，从而导致qt无法正常显示。
  thread_draw.detach();
#else
  auto thread_sink = std::thread([&]{worker_sink(yolov8);});
