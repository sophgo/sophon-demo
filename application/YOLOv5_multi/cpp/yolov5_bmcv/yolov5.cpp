...
#define INTERVAL 1000         // 压测打印fps的时间间隔（秒）
                ...
                m_queue_post("post", m_queue_size, 2)
{
  ...
  m_net_h = m_net->info()->stages[0].input_shapes[0].dims[3];
  ...
  // input attr
  img_dtype = (m_inputs[0]->tensor()->dtype == BM_INT8)? DATA_TYPE_EXT_1N_BYTE_SIGNED: DATA_TYPE_EXT_FLOAT32;
...
void YOLOv5::preprocess(std::vector<std::shared_ptr<DataDec>> &dec_images, 
  ...
  auto ret = bm_malloc_device_byte_heap(m_handle, &tensor_mem, 0, size_byte*m_batch_size);
  if (ret != BM_SUCCESS) {
  ...
  ret = bmcv_image_convert_to(m_handle, m_batch_size, converto_attr, resized_bmimgs.data(), converto_bmimgs.data());
  if (ret != BM_SUCCESS) {
      std::cerr << "图像转换失败" << std::endl;
      throw std::runtime_error("Failed to convert images");
  }
