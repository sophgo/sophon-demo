...
2. use fp_forward.py to generate qtable, --fpfwd_outputs --fpfwd_inputs args have the same function as nntc, by specifying the layer name, all corresponding layers can be assigned the corresponding fp_type. 
   ...
   Refer to the picture above; the last three convolutional layers are named 390, 416, and 442 respectively. The following command can be used as a reference:
   ```bash
   fp_forward.py xxx.mlir --fpfwd_outputs 390,416,442 --chip bm1684 -o xxx_qtable 
   ```
   Performing the last three convolutional layers and all subsequent computation layers using float precision can effectively improve the model accuracy.
...
#### 1.3.1 YOLOv8 series models
The YOLOv8 series models adopt a decoupled head structure, separating the classification and detection heads. Empirically, for this type of model, it is generally necessary to start from the convolutional layers in the coordinate regression branch and the sigmoid layers in the probability prediction branch, setting them to perform floating-point computations.
...
2. use fp_forward.py to generate qtable, --fpfwd_outputs --fpfwd_inputs args have the same function as nntc, by specifying the layer name, all corresponding layers can be assigned the corresponding fp_type. 
   ...
   ```
   **Then directly add the Sigmoid layer in the qtable**. Since the convolutional layers in the previous fp_forward process already include the operations following the Sigmoid, it is only necessary to add the following line at the end of the quantization table:
   ```bash
